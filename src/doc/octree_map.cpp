...
void OctreeNode::addColor(
  color_t c, int level, OctreeNode* parent, int paletteIndex, int levelDeep)
{
  m_parent = parent;
  if (level >= levelDeep) {
    m_leafColor.add(c);
                         ...
                         const OctreeMap* octree) const
{
  ...
  if (!m_children)
    m_children.reset(new std::array<OctreeNode, 16>());
  return (*m_children)[index].mapColor(
    r, g, b, a, mask_index, palette, level + 1, octree);
}
    ...
    for (int i = m_leavesVector.size() - 1; i >= 0; i--) {
      if (m_leavesVector.size() + auxLeavesVector.size() <= colorCount) {
        for (int j = 0; j < auxLeavesVector.size(); j++)
          m_leavesVector.push_back(
            auxLeavesVector[auxLeavesVector.size() - 1 - j]);
      ...
      m_leavesVector.back()->parent()->removeLeaves(auxLeavesVector,
                                                    m_leavesVector);
    }
    if (keepReducingMap) {
      ...
      for (int i = 0; i < auxLeavesVectorSize; i++)
        m_leavesVector.push_back(
          auxLeavesVector[auxLeavesVector.size() - 1 - i]);
      ...
      if (alpha) {
        ...
        color =
          (imageIsRGBA ?
             color :
             rgba(
               graya_getv(color), graya_getv(color), graya_getv(color), alpha));
