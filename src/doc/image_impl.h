...
public:
  ...
  ImageImpl(const ImageSpec& spec, const ImageBufferPtr& buffer)
    : Image(spec)
    , m_buffer(buffer)
  {
  ...
  void blendRect(
    int x1, int y1, int x2, int y2, color_t color, int opacity) override
  {
    fillRect(x1, y1, x2, y2, color);
  }

private:
  bool clip_rects(const Image* src,
                  int& dst_x,
                  int& dst_y,
                  int& src_x,
                  int& src_y,
...
template<>
inline void ImageImpl<BitmapTraits>::fillRect(
...
template<>
inline void ImageImpl<RgbTraits>::blendRect(
  int x1, int y1, int x2, int y2, color_t color, int opacity)
{
  address_t addr;
  int x, y;
