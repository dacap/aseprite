...
Layer* Layer::getPrevious() const
{
  if (m_parent) {
    auto it =
      std::find(m_parent->layers().begin(), m_parent->layers().end(), this);
...
Layer* Layer::getNextInWholeHierarchy() const
{
  ...
  if (Layer* next = getNext()) {
    // Go to children
    while (next->isGroup() &&
           static_cast<const LayerGroup*>(next)->layersCount() > 0) {
      Layer* firstChild = static_cast<const LayerGroup*>(next)->firstLayer();
      if (!firstChild)
        break;
      next = firstChild;
...
{
  gfx::Rect rc =
    (m_sprite ? m_sprite->gridBounds() : doc::Sprite::DefaultGridBounds());
...
CelIterator LayerImage::findCelIterator(frame_t frame)
{
  ...
  first =
    std::lower_bound(first, end, nullptr, [frame](Cel* cel, Cel*) -> bool {
...
CelIterator LayerImage::findFirstCelIteratorAfter(frame_t firstAfterFrame)
{
  ...
  first = std::lower_bound(
    first, end, nullptr, [firstAfterFrame](Cel* cel, Cel*) -> bool {
...
std::string LayerGroup::visibleLayerHierarchyAsString(
  const std::string& indent) const
{
  ...
  for (Layer* child : m_layers) {
    ...
    if (child->isGroup())
      str += static_cast<LayerGroup*>(child)->visibleLayerHierarchyAsString(
        indent + "  ");
  }
  return str;
}
