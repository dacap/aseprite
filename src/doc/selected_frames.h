...
public:
  ...
  {
    return frames::const_iterator(m_ranges.begin());
  }
  frames::const_iterator end() const
  {
    return frames::const_iterator(m_ranges.end());
  }
  ...
  frame_t firstFrame() const
  {
    return (!m_ranges.empty() ? m_ranges.front().fromFrame : -1);
  }
  frame_t lastFrame() const
  {
    return (!m_ranges.empty() ? m_ranges.back().toFrame : -1);
  }

  void displace(frame_t frameDelta);
  frames::Reversed<SelectedFrames> reversed() const
  {
    return frames::Reversed(*this);
  }
