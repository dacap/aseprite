...
{
  switch (mode) {
    case Playback::PlayAll:
      return "PlayAll";
    case Playback::PlayInLoop:
      return "PlayInLoop";
    case Playback::PlayWithoutTagsInLoop:
      return "PlayWithoutTagsInLoop";
    case Playback::PlayOnce:
      return "PlayOnce";
...
Playback::Playback(const Sprite* sprite,
  ...
  , m_forward(forward)
{
  PLAY_TRACE("--Playback-- tag=",
             (tag ? tag->name() : ""),
             "mode=",
             mode_to_string(m_playMode));
...
{
  return (!m_playing.empty() ? const_cast<Tag*>(m_playing.back()->tag) :
                               nullptr);
}
    ...
    case PlayInLoop: {
      ...
      if (tag && tag->contains(m_frame)) {
        ...
        // Change ping-pong direction
          ...
          PLAY_TRACE("    Changing direction frame=",
                     m_frame,
                     " forward=",
                     forward,
                     "->",
  ...
  while (true) {
    ...
    if (m_playing.back()->repeat > 1) {
      ...
      if (rewind && !m_playing.empty()) {
        ...
        newFrame = (frameDelta * forward < 0 ? tag->fromFrame() - 1 :
                                               tag->toFrame() + 1);
      }

      PLAY_TRACE("    After tag",
                 tag->name(),
                 "possible new frame=",
                 newFrame,
                 "forward",
      ...
      if (auto newTag = this->tag()) {
        ...
        if (m_playing.empty() && m_playMode == PlayInLoop &&
          ...
          PLAY_TRACE(
            "    Re-adding ping-pong tag", tag->name(), "frame=", m_frame);
          addTag(tag, false, getParentForward());
          return false;
        }
        else {
          PLAY_TRACE(
            "    Going outside the tag", tag->name(), "frame=", m_frame);
          return false;
        }
      }
    }
