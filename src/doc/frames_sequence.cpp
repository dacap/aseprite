...
void FramesSequence::insert(frame_t frame)
{
  ...
  if (step && (rangeStep == step || !rangeStep) &&
...
{
  return std::find_if(m_ranges.begin(),
                      m_ranges.end(),
...
frame_t FramesSequence::lowestFrame() const
{
  ...
  for (auto range : m_ranges) {
    lof =
      std::min<frame_t>(range.fromFrame, std::min<frame_t>(range.toFrame, lof));
  }
  return lof;
}
...
FramesSequence FramesSequence::makePingPong() const
{
  ...
  for (const FrameRange& range : m_ranges) {
    ...
    int step =
      (range.fromFrame < range.toFrame) - (range.fromFrame > range.toFrame);
