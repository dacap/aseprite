  ...
  switch (angle) {
    case 180:
      ...
      for (y = 0; y < src->height(); ++y)
        for (x = 0; x < src->width(); ++x)
          dst->putPixel(
            src->width() - x - 1, src->height() - y - 1, src->getPixel(x, y));
      break;
    ...
    default:
      throw std::invalid_argument(
        "Invalid angle specified to rotate the image");
  }
}
...
void fill_rect(Image* image, const gfx::Rect& rc, color_t c)
{
  ...
  if (!clip.isEmpty())
    image->fillRect(
      clip.x, clip.y, clip.x + clip.w - 1, clip.y + clip.h - 1, c);
}

void blend_rect(
  Image* image, int x1, int y1, int x2, int y2, color_t color, int opacity)
{
  ASSERT(image);
  int t;
                  ...
                  color_t color)
{
  Data data = { image, color };
  algo_ellipse(
    x1, y1, x2, y2, extraXPxs, extraYPxs, &data, (AlgoPixel)pixel_for_image);
}
                  ...
                  color_t color)
{
  Data data = { image, color };
  algo_ellipsefill(
    x1, y1, x2, y2, extraXPxs, extraYPxs, &data, (AlgoHLine)hline_for_image);
}
...
bool is_plain_image(const Image* img, color_t c)
{
  switch (img->pixelFormat()) {
    case IMAGE_RGB:
      return is_plain_image_templ<RgbTraits>(img, c);
    case IMAGE_GRAYSCALE:
      return is_plain_image_templ<GrayscaleTraits>(img, c);
    case IMAGE_INDEXED:
      return is_plain_image_templ<IndexedTraits>(img, c);
    case IMAGE_BITMAP:
      return is_plain_image_templ<BitmapTraits>(img, c);
    case IMAGE_TILEMAP:
      return is_plain_image_templ<TilemapTraits>(img, c);
  }
  return false;
}
...
int count_diff_between_images(const Image* i1, const Image* i2)
{
  ...
  switch (i1->pixelFormat()) {
    ...
    case IMAGE_TILEMAP:
      return count_diff_between_images_templ<TilemapTraits>(i1, i2);
  }
...
bool is_same_image(const Image* i1, const Image* i2)
{
  ...
  if ((cm != i2->colorMode()) || (i1->width() != i2->width()) ||
      (i1->height() != i2->height()))
    return false;
  ...
  DOC_DISPATCH_BY_COLOR_MODE_EXCLUDE_BITMAP(
    cm, is_same_image_simd_templ, i1, i2);
...
uint32_t calculate_image_hash(const Image* img, const gfx::Rect& bounds)
{
  switch (img->pixelFormat()) {
    case IMAGE_RGB:
      return calculate_image_hash_templ<RgbTraits, rgba_rgb_mask>(img, bounds);
    case IMAGE_GRAYSCALE:
      return calculate_image_hash_templ<GrayscaleTraits, graya_v_mask>(img,
                                                                       bounds);
    case IMAGE_INDEXED:
      return calculate_image_hash_templ<IndexedTraits, 0xff>(img, bounds);
    case IMAGE_BITMAP:
      return calculate_image_hash_templ<BitmapTraits, 1>(img, bounds);
  }
  ASSERT(false);
  return 0;
}
