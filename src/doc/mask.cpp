...
{
  for_each_mask_pixel(
    *this, mask, [](color_t a, color_t b) -> color_t { return a | b; });
}
...
#define ADVANCE(beg, end, o_end, cmp, op, getpixel1, getpixel)                 \
  ...
  ADVANCE(x1,
          ...
          y2,
          <=
          , ++, get_pixel(image, x1, c = beg_y1), get_pixel(image, x1, c));
  /* right */
  ADVANCE(x2,
          x1,
          y2,
          >=
          , --, get_pixel(image, x2, c = beg_y1), get_pixel(image, x2, c));
  /* top */
  ADVANCE(y1,
          y2,
          x2,
          <=
          , ++, get_pixel(image, c = beg_x1, y1), get_pixel(image, c, y1));
  /* bottom */
  ADVANCE(y2,
          y1,
          x2,
          >=
          , --, get_pixel(image, c = beg_x1, y2), get_pixel(image, c, y2));
