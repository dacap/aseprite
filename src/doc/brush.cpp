...
BrushRef Brush::cloneWithNewImages() const
{
  ...
  if (newBrush->m_maskBitmap)
    newBrush->m_maskBitmap.reset(
      Image::createCopy(newBrush->m_maskBitmap.get()));
  return newBrush;
}
...
void Brush::setImageColor(const ImageColor imageColor, const color_t color)
{
  ...
  switch (m_image->pixelFormat()) {
    ...
    case IMAGE_GRAYSCALE:
      replace_image_colors<GrayscaleTraits,
                           graya_v_mask,
                           graya_a_mask,
                           graya_a_shift>(m_image.get(),
                                          m_maskBitmap.get(),
                                          (m_mainColor ? true : false),
                                          (m_mainColor ? *m_mainColor : 0),
                                          (m_bgColor ? true : false),
...
void Brush::setCenter(const gfx::Point& center)
{
  m_center = center;
  m_bounds =
    gfx::Rect(-m_center, gfx::Size(m_image->width(), m_image->height()));
}
...
{
  draw_hline(
    reinterpret_cast<Image*>(data), x1, y, x2, BitmapTraits::max_value);
}
  ...
  if (size == 1) {
    ...
    switch (m_type) {
      ...
      case kLineBrushType: {
        ...
        draw_line(
          m_image.get(), cx, cy, cx + dx, cy + dy, BitmapTraits::max_value);
        draw_line(
          m_image.get(), cx, cy, cx - dx, cy - dy, BitmapTraits::max_value);
        break;
      }
