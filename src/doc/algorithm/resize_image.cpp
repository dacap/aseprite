  ...
  switch (method) {
    ...
    case RESIZE_METHOD_BILINEAR: {
      ...
      if (dst->pixelFormat() == IMAGE_INDEXED && (!pal || !rgbmap)) {
        resize_image(
          src, dst, RESIZE_METHOD_NEAREST_NEIGHBOR, pal, rgbmap, maskColor);
        return;
      }
      ...
      for (y = 0; y < dst->height(); ++y) {
        for (x = 0; x < dst->width(); ++x) {
          ...
          switch (dst->pixelFormat()) {
            case IMAGE_RGB: {
              int r =
                int((rgba_getr(color[0]) * u2 + rgba_getr(color[1]) * u1) * v2 +
                    (rgba_getr(color[2]) * u2 + rgba_getr(color[3]) * u1) * v1);
              int g =
                int((rgba_getg(color[0]) * u2 + rgba_getg(color[1]) * u1) * v2 +
                    (rgba_getg(color[2]) * u2 + rgba_getg(color[3]) * u1) * v1);
              int b =
                int((rgba_getb(color[0]) * u2 + rgba_getb(color[1]) * u1) * v2 +
                    (rgba_getb(color[2]) * u2 + rgba_getb(color[3]) * u1) * v1);
              int a =
                int((rgba_geta(color[0]) * u2 + rgba_geta(color[1]) * u1) * v2 +
