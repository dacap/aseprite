...
#define MASKED(u, v)                                                           \
   ...
   (!mask->bounds().contains(u, v) ||                                          \
    (mask->bitmap() &&                                                         \
     !get_pixel_fast<BitmapTraits>(                                            \
       mask->bitmap(), (u) - mask->bounds().x, (v) - mask->bounds().y))))
  ...
  switch (image->pixelFormat()) {
    case IMAGE_RGB: {
      uint32_t* address =
        reinterpret_cast<uint32_t*>(image->getPixelAddress(0, y));
    ...
    case IMAGE_GRAYSCALE: {
      uint16_t* address =
        reinterpret_cast<uint16_t*>(image->getPixelAddress(0, y));
                            ...
                            AlgoHLine proc)
{
  ...
  while (left <= right) {
    ...
    for (;;) {
      ...
      if (!c) {
        left = flooder(
          image, mask, left, y, bounds, src_color, tolerance, data, proc);
        ret = true;
        break;
      }
    }
               ...
               AlgoHLine proc)
{
  ...
  if (!contiguous) {
    switch (image->pixelFormat()) {
      case IMAGE_RGB:
        replace_color<RgbTraits>(
          image, bounds, src_color, tolerance, data, proc);
        break;
      case IMAGE_GRAYSCALE:
        replace_color<GrayscaleTraits>(
          image, bounds, src_color, tolerance, data, proc);
        break;
      case IMAGE_INDEXED:
        replace_color<IndexedTraits>(
          image, bounds, src_color, tolerance, data, proc);
        break;
      case IMAGE_TILEMAP:
        replace_color<TilemapTraits>(
          image, bounds, src_color, tolerance, data, proc);
        break;
    }
    return;
