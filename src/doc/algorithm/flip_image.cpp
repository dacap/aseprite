...
void flip_image_with_put_pixel_fast_templ(Image* image,
...
template<typename ImageTraits>
void flip_image_with_rawptr_templ(Image* image,
                                  const gfx::Rect& bounds,
                                  FlipType flipType)
{
  ...
  switch (flipType) {
    ...
    case FlipDiagonal:
      flip_image_with_put_pixel_fast_templ<ImageTraits>(
...
void flip_image(Image* image, const gfx::Rect& bounds, FlipType flipType)
{
  ...
  if (image->colorMode() == ColorMode::BITMAP) {
    return flip_image_with_put_pixel_fast_templ<BitmapTraits>(
      image, bounds, flipType);
  }

  DOC_DISPATCH_BY_COLOR_MODE_EXCLUDE_BITMAP(
    image->colorMode(), flip_image_with_rawptr_templ, image, bounds, flipType);
}

template<typename ImageTraits>
void flip_image_with_mask_templ(Image* image,
                                const Mask* mask,
                                FlipType flipType,
                                int bgcolor)
{
...
void flip_image_with_mask(Image* image,
                          const Mask* mask,
                          FlipType flipType,
                          int bgcolor)
{
