...
static void addPointsWithoutDuplicatingLastOne(int x,
                                               int y,
...
void algorithm::polygon(int vertices,
                        const int* points,
                        void* data,
                        AlgoHLine proc)
{
  ...
  for (int i = 2; i < vertices * 2; i += 2) {
    ...
    if (verts[last].x == *(points + i) &&
        verts[last].y == *(points + (i + 1))) {
      verticesAux--;
      continue;
    }
  ...
  for (y = ymin; y <= ymax; y++) {
    ...
    for (int i = 0; i < pts.size(); i++) {
      ...
      if ((y >= y1 && y < y2) || (y == ymax && y > y1 && y <= y2)) {
        polyInts[ints] =
          (int)((float)((y - y1) * (x2 - x1)) / (float)(y2 - y1) + 0.5f +
                (float)x1);
        ints++;
      }
    }
