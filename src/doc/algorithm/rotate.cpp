...
static void ase_parallelogram_map_standard(
  Image* bmp, const Image* sprite, const Image* mask, fixed xs[4], fixed ys[4]);
  ...
  Image* bmp, const Image* sprite, const Image* mask, fixed xs[4], fixed ys[4])
{
  switch (bmp->pixelFormat()) {
    case IMAGE_RGB: {
      ...
      ase_parallelogram_map<RgbTraits, RgbDelegate>(
    ...
    case IMAGE_INDEXED: {
      ...
      ase_parallelogram_map<IndexedTraits, IndexedDelegate>(
        bmp, sprite, mask, xs, ys, false, delegate);
      break;
    }
    ...
    case IMAGE_BITMAP: {
      ...
      ase_parallelogram_map<BitmapTraits, BitmapDelegate>(
        bmp, sprite, mask, xs, ys, false, delegate);
      break;
    }
  }
