...
void algo_line_perfect(
  int x1, int y1, int x2, int y2, void* data, AlgoPixel proc)
{
  bool yaxis;
...
//      would fix all cases
void algo_line_perfect_with_fix_for_line_brush(
  int x1, int y1, int x2, int y2, void* data, AlgoPixel proc)
{
  bool yaxis;
...
void algo_line_continuous(
  int x0, int y0, int x1, int y1, void* data, AlgoPixel proc)
{
...
// kLineBrushType so the whole line looks continuous without holes.
void algo_line_continuous_with_fix_for_line_brush(
  int x0, int y0, int x1, int y1, void* data, AlgoPixel proc)
{
  int dx = ABS(x1 - x0), sx = (x0 < x1 ? 1 : -1);
  int dy = -ABS(y1 - y0), sy = (y0 < y1 ? 1 : -1);
...
static int adjust_ellipse_args(
  int& x0, int& y0, int& x1, int& y1, int& hPixels, int& vPixels)
{
...
{                    // plot a limited rational Bezier segment, squared weight
  ...
  if (cur != 0.0 && w > 0.0) {                    // no straight line
    ...
    dx =
      4.0 * w * (x1 - x0) * sy * cur + xx / 2.0 + xy;  // differences 1st degree
    ...
    if (w < 0.5 && (dy > xy || dx < xy)) {  // flat ellipse, algorithm fails
      ...
      sx = std::floor((x0 + 2.0 * w * x1 + x2) * xy / 2.0 +
                      0.5);  // subdivide curve in half
      sy = std::floor((y0 + 2.0 * w * y1 + y2) * xy / 2.0 + 0.5);

      dx = std::floor((w * x1 + x0) * xy + 0.5);
      dy = std::floor((y1 * w + y0) * xy + 0.5);
      draw_quad_rational_bezier_seg(
        x0, y0, dx, dy, sx, sy, cur, data, proc);  // plot separately
  ...
  algo_line_continuous(
    x0, y0, x2, y2, data, proc);  // plot remaining needle to end
}

static void draw_rotated_ellipse_rect(
  int x0, int y0, int x1, int y1, double zd, void* data, AlgoPixel proc)
{
  int xd = x1 - x0;
  int yd = y1 - y0;
  double w = xd * yd;
  ...
  draw_quad_rational_bezier_seg(
    x0, y0 + yd, x0, y0, x0 + xd, y0, 1.0 - w, data, proc);
  draw_quad_rational_bezier_seg(
    x0, y0 + yd, x0, y1, x1 - xd, y1, w, data, proc);
  draw_quad_rational_bezier_seg(
    x1, y1 - yd, x1, y1, x1 - xd, y1, 1.0 - w, data, proc);
  draw_quad_rational_bezier_seg(
    x1, y1 - yd, x1, y0, x0 + xd, y0, w, data, proc);
}

void draw_rotated_ellipse(
  int cx, int cy, int a, int b, double angle, void* data, AlgoPixel proc)
{
  double xd = a * a;
  double yd = b * b;
...
void fill_rotated_ellipse(
  int cx, int cy, int a, int b, double angle, void* data, AlgoHLine proc)
{
  struct Rows {
    int y0;
  ...
  draw_rotated_ellipse_rect(
    cx - a, cy - b, cx + a, cy + b, zd, &rows, [](int x, int y, void* data) {
      Rows* rows = (Rows*)data;
      rows->update(x, y);
    });
...
#define DIST(x, y) (sqrt((x) * (x) + (y) * (y)))
  npts = (int)(sqrt(DIST(x1 - x0, y1 - y0) + DIST(x2 - x1, y2 - y1) +
                    DIST(x3 - x2, y3 - y2)) *
               1.2);
  if (npts > MAX_POINTS)
    npts = MAX_POINTS;
