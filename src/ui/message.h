...
public:
  ...
  void setPropagateToChildren(const bool state)
  {
    setFlag(PropagateToChildren, state);
  }
  void setPropagateToParent(const bool state)
  {
    setFlag(PropagateToParent, state);
  }
...
private:
  ...
  Widget*
    m_commonAncestor;  // Common ancestor between the Leave <-> Enter messages
