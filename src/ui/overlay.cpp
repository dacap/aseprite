...
void Overlay::captureOverlappedArea()
{
  ...
  if (!m_overlap) {
    ...
    m_overlap = os::instance()->makeSurface(
      m_surface->width(), m_surface->height(), displaySurface->colorSpace());
  }

  os::SurfaceLock lock(m_overlap.get());
  displaySurface->blitTo(m_overlap.get(),
                         m_pos.x,
                         m_pos.y,
                         0,
                         0,
...
void Overlay::restoreOverlappedArea(const gfx::Rect& restoreBounds)
{
  ...
  os::SurfaceLock lock(m_overlap.get());
  m_overlap->blitTo(m_captured.get(),
                    0,
                    0,
                    m_pos.x,
                    m_pos.y,
