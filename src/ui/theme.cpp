                    ...
                    std::function<void(const Style::Layer&)> callback)
{
  ...
  for (const auto& layer : style->layers()) {
    ...
    if ((!layer.flags() || (layer.flags() & flags) == layer.flags()) &&
        (!bestLayer ||
         (bestLayer &&
          compare_layer_flags(bestLayer->flags(), layer.flags()) <= 0))) {
      bestLayer = &layer;
    }
  }
...
{
  for_each_layer(
    PaintWidgetPartInfo::getStyleFlagsForWidget(widget), style, callback);
}
...
getDrawSurfaceFunction(Graphics* g, os::Surface* sheet, gfx::Color color)
{
  if (color != gfx::ColorNone)
    return
      [g, sheet, color](int srcx, int srcy, int dstx, int dsty, int w, int h) {
...
void Theme::setDecorativeWidgetBounds(Widget* widget)
{
  switch (widget->type()) {
    case kWindowTitleLabelWidget: {
      ...
      labelBounds.offset(windowBounds.x + margin.left(),
      ...
      if (layer.color() != gfx::ColorNone) {
        ...
        if (layer.align() & WORDWRAP) {
          ...
          if (layer.align() & LEFT)
            ...
            pt.x = CALC_FOR_CENTER(
              rc.x + padding.left(), rc.w - padding.width(), textSize.w);
          }
          ...
          if (layer.align() & TOP)
            ...
            pt.y = CALC_FOR_CENTER(
              rc.y + padding.top(), rc.h - padding.height(), textSize.h);
          }
    ...
    case Style::Layer::Type::kIcon: {
      ...
      if (icon) {
        ...
        if (layer.align() & LEFT)
          ...
          pt.x = CALC_FOR_CENTER(
            rc.x + padding.left(), rc.w - padding.width(), iconSize.w);
        }
        ...
        if (layer.align() & TOP)
          ...
          pt.y = CALC_FOR_CENTER(
            rc.y + padding.top(), rc.h - padding.height(), iconSize.h);
        }
                         ...
                         int& iconAlign)
{
  ...
  switch (layer.type()) {
    ...
    case Style::Layer::Type::kBorder:
      if (layer.spriteSheet() && !layer.spriteBounds().isEmpty()) {
        if (!layer.slicesBounds().isEmpty()) {
          ...
          borderHint.right(
            std::max(borderHint.right(),
                     layer.spriteBounds().w - layer.slicesBounds().x2()));
          borderHint.bottom(
            std::max(borderHint.bottom(),
                     layer.spriteBounds().h - layer.slicesBounds().y2()));
        }
        else {
    ...
    case Style::Layer::Type::kIcon: {
      ...
      if (icon) {
        iconHint.w =
          std::max(iconHint.w, icon->width() + ABS(layer.offset().x));
        iconHint.h =
          std::max(iconHint.h, icon->height() + ABS(layer.offset().y));
                       ...
                       gfx::Size& bottomRight)
{
  ...
  for_each_layer(widget,
                 style,
                 [&topLeft, &center, &bottomRight](const Style::Layer& layer) {
                   if (layer.spriteSheet() && !layer.spriteBounds().isEmpty() &&
                       !layer.slicesBounds().isEmpty()) {
                     ...
                     bottomRight.w =
                       std::max(bottomRight.w, sprite.w - slices.x2());
                     bottomRight.h =
                       std::max(bottomRight.h, sprite.h - slices.y2());
                   }
                 });
}
                              ...
                              int& textAlign)
{
  ...
  for_each_layer(widget,
                 ...
                 [this,
                  ...
                  &textAlign,
                  &iconHint,
                  &iconAlign](const Style::Layer& layer) {
                   measureLayer(widget,
                                style,
                                layer,
                                borderHint,
