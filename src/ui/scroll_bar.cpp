...
void ScrollBar::onPaint(PaintEvent& ev)
{
  ...
  theme()->paintScrollBar(
    ev.graphics(), this, style(), thumbStyle(), clientBounds(), thumbBounds);
}

void ScrollBar::getScrollBarInfo(int* _pos,
                                 int* _len,
                                 int* _bar_size,
                                 int* _viewport_size)
{
  ...
  if (m_size <= viewport_size) {
    ...
    len = std::clamp(
      len,
      std::min(theme()->getScrollbarSize() * 2 - border_width, bar_size),
      bar_size);
