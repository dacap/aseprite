...
void Graphics::drawLine(gfx::Color color,
                        const gfx::Point& _a,
                        const gfx::Point& _b)
{
                           ...
                           const ui::Paint* paint)
{
  ...
  m_surface->drawSurface(
    surface, srcRect, gfx::Rect(dstRect).offset(m_dx, m_dy), sampling, paint);
}
...
void Graphics::drawRgbaSurface(
  os::Surface* surface, int srcx, int srcy, int dstx, int dsty, int w, int h)
{
  ...
  m_surface->drawRgbaSurface(
    surface, srcx, srcy, m_dx + dstx, m_dy + dsty, w, h);
}
                               ...
                               const ui::Paint* paint)
{
  ...
  os::SurfaceLock lockSrc(surface);
  os::SurfaceLock lockDst(m_surface.get());
  m_surface->drawSurfaceNine(
    surface, src, center, displacedDst, drawCenter, paint);
}
                        ...
                        os::DrawTextDelegate* delegate)
{
  ...
  os::SurfaceLock lock(m_surface.get());
  gfx::Rect textBounds = os::draw_text(
    m_surface.get(), m_font.get(), str, fg, bg, pt.x, pt.y, delegate);
...
public:
  ...
  void postDrawChar(const gfx::Rect& charBounds) override
  {
    if (!gfx::is_transparent(m_underscoreColor)) {
      ...
      if (m_font->type() ==
          os::FontType::
            FreeType)  // TODO use other method to locate the underline
        dy += guiscale();
...
{
  return gfx::Size(Graphics::measureUITextLength(str, m_font.get()),
                   m_font->height());
}
...
int Graphics::measureUITextLength(const std::string& str, os::Font* font)
{
  ...
  os::draw_text(
    nullptr, font, str, gfx::ColorNone, gfx::ColorNone, 0, 0, &delegate);
