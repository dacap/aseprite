...
public:
  WindowTitleLabel(const std::string& text)
    : Label(text)
  {
    setDecorative(true);
...
public:
  WindowCloseButton()
    : ButtonBase("", kWindowCloseButtonWidget, kButtonWidget, kButtonWidget)
  {
    setDecorative(true);
    initTheme();
  }
    ...
    case kMouseMoveMessage:
      ...
      if (hasCapture()) {
        ...
        if (m_hitTest == HitTestCaption) {
          gfx::Point pos =
            clickedWindowPos->origin() + mousePos - clickedMousePos;
  ...
  if (text().empty()) {
    ...
    if (!m_titleLabel) {
      ...
      m_titleLabel->setBounds(
        gfx::Rect(m_titleLabel->bounds()).setSize(m_titleLabel->sizeHint()));
    }
  }
}
