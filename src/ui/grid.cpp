...
void Grid::calculateCellSize(int start,
                             int span,
                             const std::vector<Strip>& strip,
                             int& size)
{
  int i, j;
  int gap = (&strip == &m_colstrip ? m_colgap : m_rowgap);
      ...
      if (cell->child != nullptr) {
        if (cell->parent == nullptr) {
          ...
          if (!(cell->child->hasFlags(HIDDEN))) {
            ...
            cell->w =
              reqSize.w - (cell->hspan - 1) * (this->childSpacing() + m_colgap);
            cell->h =
              reqSize.h - (cell->vspan - 1) * (this->childSpacing() + m_rowgap);
        ...
        if (cell->child != nullptr && cell->parent == nullptr &&
          ...
          if (cell_span == current_span) {
            ...
            for (i = col; i < col + cell_span; ++i)
              max_expand_count =
                std::max(max_expand_count, colstrip[i].expand_count);

            int expand =
              0;  // How many columns have the maximum value of "expand_count"
            int last_expand =
              0;  // This variable is used to add the remainder space to the last column
            for (i = col; i < col + cell_span; ++i) {
...
void Grid::distributeSize(const gfx::Rect& rect)
{
  ...
  distributeStripSize(
    m_colstrip, rect.w, border().width(), m_same_width_columns);
