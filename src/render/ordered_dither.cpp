  ...
  int r1, int g1, int b1, int a1, int r2, int g2, int b2, int a2)
{
  ...
  if (a1 && a2) {
    result += int(std::abs(r1 - r2) * 2126 + std::abs(g1 - g2) * 7152 +
                  std::abs(b1 - b2) * 722);
  }
                                 ...
                                 TaskDelegate* delegate)
{
  ...
  if (algorithm.dimensions() == 1) {
    ...
    auto dstIt =
      doc::get_pixel_address_fast<doc::IndexedTraits>(dstImage, 0, 0);
    ...
    for (int y = 0; y < h; ++y) {
      if (zigZag && (y & 1)) {  // Odd row: go from right-to-left
        ...
        for (int x = w - 1; x >= 0; --x, --dstIt) {
          ASSERT(dstIt == doc::get_pixel_address_fast<doc::IndexedTraits>(
                            dstImage, x, y));
