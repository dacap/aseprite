...
typedef testing::Types<RgbTraits, GrayscaleTraits, IndexedTraits>
  ImageAllTraits;
...
TEST(Render, Basic)
{
  ...
  doc->sprites().add(
    Sprite::MakeStdSprite(ImageSpec(ColorMode::INDEXED, 2, 2)));
...
TYPED_TEST(RenderAllModes, CheckDefaultBackgroundMode)
{
  ...
  std::shared_ptr<Document> doc = std::make_shared<Document>();
  doc->sprites().add(
    Sprite::MakeStdSprite(ImageSpec(ImageTraits::color_mode, 2, 2)));
...
TEST(Render, CheckeredBackground)
{
  std::shared_ptr<Document> doc = std::make_shared<Document>();
  doc->sprites().add(
    Sprite::MakeStdSprite(ImageSpec(ColorMode::INDEXED, 4, 4)));
...
TEST(Render, ZoomAndDstBounds)
{
  ...
  std::shared_ptr<Document> doc = std::make_shared<Document>();
  doc->sprites().add(
    Sprite::MakeStdSprite(ImageSpec(ColorMode::INDEXED, 3, 3)));
  Image* src = doc->sprite()->root()->firstLayer()->cel(0)->image();
  clear_image(src, 0);
  ...
  render.renderSprite(
    dst.get(), doc->sprite(), frame_t(0), gfx::Clip(1, 1, 0, 0, 2, 2));
...
TEST(Render, BugWithMultiplesOf3ZoomFactors)
{
  ...
  int zooms[] = { 1,  2,  3,  4,  6,  8,  9,  12, 15, 16, 18,
                  21, 24, 27, 30, 32, 33, 36, 39, 42, 45, 48,
