...
void HueSaturationFilter::applyToRgba(FilterManager* filterMgr)
{
  ...
  FILTER_LOOP_THROUGH_ROW_BEGIN(uint32_t)
  {
    ...
    if (newPal) {
      int i = pal->findExactMatch(
        rgba_getr(c), rgba_getg(c), rgba_getb(c), rgba_geta(c), -1);
      if (i >= 0)
        c = newPal->getEntry(i);
    }
    else {
                                            ...
                                            bool multiply)
{
  ...
  double s =
    (multiply ? hsl.saturation() * (1.0 + m_s) : hsl.saturation() + m_s);
...
void HueSaturationFilter::applyFilterToRgb(const Target target,
                                           doc::color_t& color)
{
  switch (m_mode) {
    case Mode::HSV_MUL:
      applyFilterToRgbT<gfx::Hsv, &gfx::Hsv::value, &gfx::Hsv::value>(
        target, color, true);
      break;
    case Mode::HSL_MUL:
      applyFilterToRgbT<gfx::Hsl, &gfx::Hsl::lightness, &gfx::Hsl::lightness>(
        target, color, true);
      break;
    case Mode::HSV_ADD:
      applyFilterToRgbT<gfx::Hsv, &gfx::Hsv::value, &gfx::Hsv::value>(
