                                 ...
                                 int indentSpaces)
{
  ...
  if (hasOptions)
    std::cout << indent
              << "  std::vector<OptionBase*> optionList() const override;\n";

  if (hasSections)
    std::cout << indent
              << "  std::vector<Section*> sectionList() const override;\n";
                                  ...
                                  const char* section)
{
  ...
  std::cout << "Section* " << prefix << className
            << "::section(const char* id)\n"
            << "{\n";
  ...
  while (child) {
    if (child->Value()) {
      ...
      if (name == "section") {
        std::string memberName = convert_xmlid_to_cppid(childId, false);
        std::cout << "  if (std::strcmp(id, " << memberName
                  << ".name()) == 0) return &" << memberName << ";\n";
      }
    }
    child = child->NextSiblingElement();
  }
  ...
  std::cout << "OptionBase* " << prefix << className
    ...
    if (child->Value()) {
      ...
      if (name == "section") {
        ...
        print_pref_class_impl(
          child, className + "::", childClassName, child->Attribute("id"));
      }
    }
    child = child->NextSiblingElement();
  }
