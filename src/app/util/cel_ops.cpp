...
void mask_image(Image* image, Image* bitmap)
{
  ...
  switch (image->pixelFormat()) {
    case IMAGE_RGB:
      return mask_image_templ<RgbTraits>(image, bitmap);
    case IMAGE_GRAYSCALE:
      return mask_image_templ<GrayscaleTraits>(image, bitmap);
    case IMAGE_INDEXED:
      return mask_image_templ<IndexedTraits>(image, bitmap);
  }
}
                                    ...
                                    gfx::Region& output)
{
  ...
  switch (a->pixelFormat()) {
    case IMAGE_RGB:
      create_region_with_differences_templ<RgbTraits>(a, b, bounds, output);
      break;
    case IMAGE_GRAYSCALE:
      create_region_with_differences_templ<GrayscaleTraits>(
        a, b, bounds, output);
      break;
    case IMAGE_INDEXED:
                     ...
                     const frame_t dstFrame)
{
  ...
  doc::PixelFormat dstPixelFormat =
    (dstLayer->isTilemap() ? IMAGE_TILEMAP : dstSprite->pixelFormat());
  ...
  // Image -> Tilemap (we'll need to generate new tilesets)
    ...
    ImageRef tmpImage(
      Image::create(IMAGE_RGB, srcImage->width(), srcImage->height()));
    tmpImage->clear(0);
                                     ...
                                     doc::ImageRef& newTilemap)
{
  ...
  if (!newTilemap) {
    newTilemap.reset(
      doc::Image::create(IMAGE_TILEMAP, tilemapBounds.w, tilemapBounds.h));
  ...
  for (const gfx::Point& tilePt :
       grid.tilesInCanvasRegion(gfx::Region(canvasBounds))) {
    ...
    // some checks to the operation.
    {
      ...
      ASSERT((u >= 0) && (v >= 0) && (u < newTilemap->width()) &&
             (v < newTilemap->height()));
                               ...
                               const gfx::Region& forceRegion)
{
  ...
  doc::LayerTilemap* tilemapLayer =
    static_cast<doc::LayerTilemap*>(cel->layer());
  ...
  if (tilesetMode == TilesetMode::Auto || tilesetMode == TilesetMode::Stack) {
    ...
    doc::ImageRef newTilemap(doc::Image::create(
      IMAGE_TILEMAP, newTilemapBounds.w, newTilemapBounds.h));
    ...
    for (const gfx::Point& tilePt : grid.tilesInCanvasRegion(regionToPatch)) {
      ...
      const doc::tile_index ti =
        (t != doc::notile ? doc::tile_geti(t) : doc::notile);
      ...
      if (find_tile(tileset, tileImage, tileIndex, tileFlag)) {
      ...
      else if (tilesetMode == TilesetMode::Auto && t != doc::notile &&
        ...
        tileIndex = ti;
        cmds->executeAndAdd(new cmd::CopyTileRegion(
      ...
      if (t != tile) {
        ...
        if (tilesetMode == TilesetMode::Auto && tile != doc::notile &&
    ...
    if (newTilemap->width() != cel->image()->width() ||
      ...
      cmds->executeAndAdd(new cmd::CopyRegion(
        cel->image(), newTilemap.get(), tilePtsRgn, gfx::Point(0, 0)));
    }

    // Remove unused tiles
    if (tilesetMode == TilesetMode::Auto) {
      remove_unused_tiles_from_tileset(
        cmds, tileset, tilesHistogram, modifiedTileIndexes);
    }
    ...
    for (const gfx::Point& tilePt : grid.tilesInCanvasRegion(region)) {
      ...
      if (tf & doc::tile_f_xflip) {
        doc::algorithm::flip_image(
          tileImage.get(), tileImageBounds, doc::algorithm::FlipHorizontal);
      ...
      if (tf & doc::tile_f_yflip) {
        doc::algorithm::flip_image(
          tileImage.get(), tileImageBounds, doc::algorithm::FlipVertical);
      ...
      if (tf & doc::tile_f_dflip) {
        doc::algorithm::flip_image(
          tileImage.get(), tileImageBounds, doc::algorithm::FlipDiagonal);
    ...
    modify_tilemap_cel_region(
      ...
      [bgcolor, mask](const doc::ImageRef& origTile,
        ...
        doc::algorithm::fill_selection(
          modified.get(), tileBoundsInCanvas, mask, bgcolor, nullptr);
        return modified;
      });
  }
  else {
  ...
  const std::vector<bool>& modifiedTileIndexes)
{
  ...
  for (; ti < remap.size(); ++ti) {
    ...
    if (ti < tilesHistogram.size() && tilesHistogram[ti] == 0 &&
  ...
  for (int i = 0; i < m; ++i) {
    ...
    if (picks[i]) {
      ...
      while (tileset->size() < i)
        cmds->executeAndAdd(
          new cmd::AddTile(tileset, tileset->makeEmptyTile()));
