...
bool get_shrink_rect(
  int* x1, int* y1, int* x2, int* y2, Image* image, color_t refpixel)
{
#define SHRINK_SIDE(                                                           \
  u_begin, u_op, u_final, u_add, v_begin, v_op, v_final, v_add, U, V, var)     \
  do {                                                                         \
  ...
  *y2 = image->height() - 1;
  ...
  SHRINK_SIDE(
    0, <, image->width(), ++, 0, <, image->height(), ++, u, v, (*x1)++);

  SHRINK_SIDE(
    0, <, image->height(), ++, 0, <, image->width(), ++, v, u, (*y1)++);

  SHRINK_SIDE(
    image->width() - 1, >, 0, --, 0, <, image->height(), ++, u, v, (*x2)--);

  SHRINK_SIDE(
    image->height() - 1, >, 0, --, 0, <, image->width(), ++, v, u, (*y2)--);
...
bool get_shrink_rect2(
  int* x1, int* y1, int* x2, int* y2, Image* image, Image* refimage)
{
#define SHRINK_SIDE(                                                           \
  u_begin, u_op, u_final, u_add, v_begin, v_op, v_final, v_add, U, V, var)     \
  do {                                                                         \
...
{
  const color_t probableRefColor1 = get_pixel(image, 0, 0);
  const color_t probableRefColor2 =
    get_pixel(image, image->width() - 1, image->height() - 1);
  ...
  if (analize_if_image_has_solid_borders(
    ...
    else if (isBorder1Solid && isBorder2Solid &&
  ...
  for (frame_t frame(0); frame < sprite->totalFrames(); ++frame) {
    ...
    if (byGrid) {
      ...
      gfx::Point posTopLeft =
        snap_to_grid(gridBounds, bounds.origin(), PreferSnapTo::FloorGrid);
      gfx::Point posBottomRight =
        snap_to_grid(gridBounds, bounds.point2(), PreferSnapTo::CeilGrid);
