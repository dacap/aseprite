                                ...
                                bool merged)
{
  ...
  if (merged || site.layer()->isTilemap()) {
    ...
    if (merged)
      render.renderSprite(
        dst.get(), srcSprite, site.frame(), gfx::Clip(0, 0, srcBounds));
    else {
...
doc::Image* new_tilemap_from_mask(const Site& site, const doc::Mask* srcMask)
{
  ...
  std::unique_ptr<Image> dst(
    Image::create(IMAGE_TILEMAP, srcTilesBounds.w, srcTilesBounds.h));
  if (!dst)
    return nullptr;
  ...
  if (srcMaskBitmap) {
    ...
    for (int v = 0; v < srcBounds.h; ++v) {
      for (int u = 0; u < srcBounds.w; ++u, ++mask_it) {
        ...
        if (*mask_it) {
          gfx::Point srcPt =
            grid.canvasToTile(gfx::Point(srcBounds.x + u, srcBounds.y + v));
          gfx::Point dstPt = srcPt - originPt;

          if (dst->bounds().contains(dstPt) &&
              srcCel->image()->bounds().contains(srcPt)) {
            dst->putPixel(
              dstPt.x, dstPt.y, srcCel->image()->getPixel(srcPt.x, srcPt.y));
          }
        }
      }
    }
