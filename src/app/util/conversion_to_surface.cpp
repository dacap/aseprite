...
template<>
uint32_t convert_color_to_surface<RgbTraits, os::kRgbaSurfaceFormat>(
  color_t c,
  const Palette* palette,
  const ImageSpec& spec,
  const os::SurfaceFormatData* fd)
                                    ...
                                    const os::SurfaceFormatData* fd)
{
  ...
  for (int v = 0; v < h; ++v, ++dst_y) {
    ...
    for (int u = 0; u < w; ++u) {
      ...
      *dst_address =
        convert_color_to_surface<ImageTraits, os::kRgbaSurfaceFormat>(
          *src_it, palette, image->spec(), fd);
      ++dst_address;
      ++src_it;
    }
  }
