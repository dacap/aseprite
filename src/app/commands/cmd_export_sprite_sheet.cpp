...
ConstraintType constraint_type_from_params(
...
void insert_layers_to_selected_layers(Layer* layer,
...
std::unique_ptr<Doc> generate_sprite_sheet(
  ...
  base::task_token& token)
{
  std::unique_ptr<Doc> newDocument(generate_sprite_sheet_from_params(
...
public:
    ...
    , m_tagnameFormat(params.tagnameFormat())
  {
    ...
    closeSpriteSection()->Click.connect(
      [this] { onCloseSection(kSectionSprite); });
    closeBordersSection()->Click.connect(
      [this] { onCloseSection(kSectionBorders); });
    closeOutputSection()->Click.connect(
      [this] { onCloseSection(kSectionOutput); });
    ...
    constraintType()->addItem(
      Strings::export_sprite_sheet_constraint_fixed_none());
    constraintType()->addItem(
      Strings::export_sprite_sheet_constraint_fixed_cols());
    constraintType()->addItem(
      Strings::export_sprite_sheet_constraint_fixed_rows());
    constraintType()->addItem(
      Strings::export_sprite_sheet_constraint_fixed_width());
    constraintType()->addItem(
      Strings::export_sprite_sheet_constraint_fixed_height());
    constraintType()->addItem(
      Strings::export_sprite_sheet_constraint_fixed_size());
    ...
    static_assert(kSource_Sprite == 0 && kSource_SpriteGrid == 1 &&
                    kSource_Tilesets == 2,
    ...
    gridTrimEnabled()->setSelected(
      (trimSpriteEnabled()->isSelected() || trimEnabled()->isSelected()) &&
      params.trimByGrid());
    ...
    if (m_filename.empty() || m_filename == kSpecifiedFilename) {
      ...
      if (base::utf8_icmp(base::get_file_extension(site.document()->filename()),
                          defExt) == 0)
    ...
    dataFilenameFormat()->Change.connect(
      [this] { onDataFilenameFormatChange(); });
    dataTagnameFormat()->Change.connect(
      [this] { onDataTagnameFormatChange(); });
  ...
  void updateParams(ExportSpriteSheetParams& params)
  {
    ...
    params.splitGrid(source()->getSelectedItemIndex() ==
                     int(kSource_SpriteGrid));
    params.fromTilesets(source()->getSelectedItemIndex() ==
                        int(kSource_Tilesets));
  }
...
private:
  ...
  void onChangeSection()
  {
    ...
    sectionOutputSeparator()->setVisible(output &&
                                         (layout || sprite || borders));
  ...
  int widthValue() const
  {
    if ((spriteSheetTypeValue() == app::SpriteSheetType::Rows ||
        ...
        (constraintType()->getSelectedItemIndex() ==
           (int)kConstraintType_Width ||
         constraintType()->getSelectedItemIndex() ==
           (int)kConstraintType_Size)) {
      return widthConstraint()->textInt();
    }
    else
      return 0;
  ...
  void generatePreview()
  {
    ...
    task->run([this, params](base::task_token& token) {
                                       ...
                                       base::task_token& token)
  {
    ...
    Doc* newDocument =
      generate_sprite_sheet(m_exporter, &tmpCtx, m_site, params, false, token)
        .release();
    if (!newDocument)
      return;
    ...
    ui::execute_from_ui_thread([this, newDocument, executionID] {
  ...
  void openGeneratedSpriteSheet(Doc* newDocument, int executionID)
  {
    ...
    if (!m_spriteSheet) {
      ...
      auto spriteSheetLay =
        static_cast<LayerImage*>(m_spriteSheet->sprite()->root()->firstLayer());
      auto newDocLay =
        static_cast<LayerImage*>(newDocument->sprite()->root()->firstLayer());
...
void ExportSpriteSheetCommand::onExecute(Context* context)
{
  ...
  {
    auto& defPref = (docPref.spriteSheet.defined() ?
                       docPref :
  ...
  if (context->isUIAvailable() && askOverwrite) {
    if (!ask_overwrite(
          true, params.textureFilename(), true, params.dataFilename()))
  ...
  if (context->isUIAvailable()) {
    ...
    base::task_token token;
    newDocument =
      generate_sprite_sheet(exporter, context, site, params, true, token);
    if (!newDocument)
      return;
  }
