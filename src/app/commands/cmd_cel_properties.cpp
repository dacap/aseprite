...
public:
  ...
  void setCel(Doc* doc, Cel* cel)
  {
    ...
    g_window->expandWindow(
      gfx::Size(g_window->bounds().w, g_window->sizeHint().h));
    updateFromCel();
  }
      ...
      try {
        ...
        for (Cel* cel : sprite->uniqueCels(range.selectedFrames())) {
          if (range.contains(cel->layer())) {
            if (opacityChanged && !cel->layer()->isBackground() &&
  ...
  void updateFromCel()
  {
    ...
    if (count > 0) {
      if (m_cel) {
        ...
        m_userDataView.color()->setColor(Color::fromRgb(
