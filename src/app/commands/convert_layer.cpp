...
void ConvertLayerCommand::onExecute(Context* ctx)
{
  ...
  if (params().to() == ConvertLayerParam::Tilemap && ctx->isUIAvailable() &&
      params().ui() &&
      // Background or Transparent Layer -> Tilemap
      (srcLayer->isImage() &&
       (srcLayer->isBackground() || srcLayer->isTransparent()))) {
    ...
    switch (params().to()) {
      ...
      case ConvertLayerParam::Layer:
        ...
        // Tilemap -> Layer
          ...
          newLayer->setContinuous(srcLayer->isContinuous());
          newLayer->setBlendMode(
            static_cast<LayerImage*>(srcLayer)->blendMode());
  ...
  for (Cel* srcCel : srcCels) {
    ...
    if (linkedSrcCel) {
      ...
      if (it != linkedCels.end()) {
        tx(new cmd::CopyCel(
          newLayer, linkedSrcCel->frame(), newLayer, frame, true));
        continue;
      }
    }

    Cel* newCel =
      create_cel_copy(tx, srcCel, srcLayer->sprite(), newLayer, frame);
...
std::string ConvertLayerCommand::onGetFriendlyName() const
{
  switch (params().to()) {
    case ConvertLayerParam::Background:
      return Strings::commands_ConvertLayer_Background();
      break;
    case ConvertLayerParam::Layer:
      return Strings::commands_ConvertLayer_Layer();
      break;
    case ConvertLayerParam::Tilemap:
      return Strings::commands_ConvertLayer_Tilemap();
      break;
    default:
      return Strings::commands_ConvertLayer();
  }
}
