  ...
  void onPaint(PaintEvent& ev) override
  {
    ...
    if (m_fileContent) {
      ...
      for (int i = 0; i < nlines; ++i) {
        ...
        g->drawText(
          line, fg, bg, gfx::Point(pt.x + icon->width() + linesVp.w, pt.y));
    ...
    if (m_fileContent) {
      if (m_maxLineWidth == 0) {
        ...
        for (const uint8_t* line : m_fileContent->lines) {
          ...
          m_maxLineWidth =
            std::max(m_maxLineWidth, Graphics::measureUITextLength(tmp, f));
        }
      }
...
private:
  int getLineNumberColumnWidth() const
  {
    auto f = font();
    int nlines = (m_fileContent ? m_fileContent->lines.size() : 0);
    return Graphics::measureUITextLength(base::convert_to<std::string>(nlines),
                                         f) +
...
public:
  ...
  public:
      ...
      , m_stackLevel(stackLevel)
    {
      ...
      setText(
        fmt::format("{}:{}: {}", base::get_file_name(m_fn), m_ln, lineContent));
    }
...
public:
  Debugger()
  {
    ...
    m_evalEntry.Execute.connect(
      [this](const std::string& expr) { onEvalExpression(expr); });
  ...
  {
    bool isRunning =
      (m_state == State::WaitingHook || m_state == State::ProcessingCommand);
...
private:
  ...
  {
    if (auto item =
          dynamic_cast<StacktraceBox::Item*>(m_stacktrace.getSelectedChild())) {
