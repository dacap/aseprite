...
struct SpriteSizeParams : public NewParams {
  ...
  Param<ResizeMethod> method{ this,
...
protected:
  ...
  void onSpriteJob(Tx& tx) override
  {
    ...
    for (Cel* cel :
         sprite()
           ->uniqueCels()) {  // TODO add size() member function to CelsRange
      (void)cel;
      ++img_count;
    }
    ...
    for (Cel* cel : sprite()->uniqueCels()) {
      ...
      if (cel->layer()->isTilemap()) {
        ...
        gfx::Rect newBounds(
          cel->x() * scale.w, cel->y() * scale.h, canvasSize.w, canvasSize.h);
    ...
    for (auto& slice : sprite()->slices()) {
      for (auto& k : *slice) {
        ...
        if (newKey.hasPivot())
          newKey.setPivot(
            gfx::Point(scale_x(newKey.pivot().x), scale_y(newKey.pivot().y)));
...
public:
  SpriteSizeWindow(Context* ctx, const SpriteSizeParams& params)
    : m_ctx(ctx)
  {
    ...
    if (params.method.isSet())
      ...
      resize_method = get_config_int(
        "SpriteSize", "Method", doc::algorithm::RESIZE_METHOD_NEAREST_NEIGHBOR);
...
void SpriteSizeCommand::onExecute(Context* context)
{
  ...
  if (ui) {
    ...
    set_config_bool(
      "SpriteSize", "LockRatio", window.lockRatio()->isSelected());
  }
