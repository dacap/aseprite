...
public:
    ...
    , m_isOrigMaskVisible(reader.document()->isMaskVisible())
  {
    ...
    m_buttonColor = new ColorButton(
      params.color(), reader.sprite()->pixelFormat(), ColorButtonOptions());
    ...
    button_cancel->Click.connect(
      [this, button_cancel] { closeWindow(button_cancel); });
  ...
  gen::SelectionMode getSelectionMode() const
  {
    return m_selMode->selectionMode();
  }
...
void MaskByColorCommand::onExecute(Context* context)
{
  ...
  if (apply) {
    int color =
      color_utils::color_for_image(params.color(), sprite->pixelFormat());
