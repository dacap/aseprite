...
{
  if (item) {
    std::string buf = fmt::format("{} {} Component",
...
void FilterTargetButtons::onItemChange(Item* item)
{
  ...
  if (m_cels->isSelected()) {
    ...
    celsTarget =  // Switch cels target
      (m_celsTarget == CelsTarget::Selected ? CelsTarget::All :
                                              CelsTarget::Selected);
  }
...
std::string FilterTargetButtons::getCelsTargetText() const
{
  switch (m_celsTarget) {
    case CelsTarget::Selected:
      return Strings::filters_selected_cels();
    case CelsTarget::All:
      return Strings::filters_all_cels();
  }
  return std::string();
}
...
std::string FilterTargetButtons::getCelsTargetTooltip() const
{
  switch (m_celsTarget) {
    case CelsTarget::Selected:
      return Strings::filters_selected_cels_tooltip();
    case CelsTarget::All:
      return Strings::filters_all_cels_tooltip();
  }
  return std::string();
}
