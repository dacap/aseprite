...
ColorCurveEditor::ColorCurveEditor(const ColorCurve& curve,
  ...
  switch (msg->type()) {
    case kKeyDownMessage: {
      switch (static_cast<KeyMessage*>(msg)->scancode()) {
        ...
        case kKeyDel: {
          if (gfx::Point* point =
                getClosestPoint(screenToView(mousePosInDisplay())))
            removePoint(*point);
          break;
        }
        ...
        case STATUS_MOVING_POINT:
          if (m_editPoint) {
            ...
            *m_editPoint = screenToView(mousePos);
            m_editPoint->x =
              std::clamp(m_editPoint->x, m_viewBounds.x, m_viewBounds.x2() - 1);
            m_editPoint->y =
              std::clamp(m_editPoint->y, m_viewBounds.y, m_viewBounds.y2() - 1);
...
void ColorCurveEditor::onPaint(ui::PaintEvent& ev)
{
  ...
  m_curve.getValues(
    m_viewBounds.x, m_viewBounds.x + m_viewBounds.w - 1, values);
