...
private:
  ...
  AlertPtr
    m_window;  // Alert for the user to cancel the filter-progress if he wants.
};
...
private:
  ...
  std::mutex
    m_mutex;  // Mutex to access to 'pos', 'done' and 'cancelled' fields in different threads.
...
void FilterWorker::run()
{
  ...
  if (!m_error.empty()) {
    ...
    StatusBar::instance()->showTip(
      2500, Strings::statusbar_tips_filter_no_unlocked_layer());
  }
}
