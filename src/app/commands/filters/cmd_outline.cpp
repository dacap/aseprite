...
struct OutlineParams : public NewParams {
  ...
  Param<filters::OutlineFilter::Place> place{ this,
                                              OutlineFilter::Place::Outside,
                                              "place" };
  Param<filters::OutlineFilter::Matrix> matrix{ this,
                                                OutlineFilter::Matrix::Circle,
                                                "matrix" };
...
public:
  OutlineFilterWrapper(Layer* layer)
    : m_layer(layer)
  {
  }
...
public:
    ...
    , m_filter(filter)
  {
    ...
    m_panel.outlineType()->ItemChange.connect(
      [this](ButtonSet::Item*) { onMatrixTypeChange(); });
...
private:
  void updateButtonsFromMatrix()
  {
    ...
    int commonMatrix = -1;
    switch (matrix) {
      case OutlineFilter::Matrix::Circle:
        commonMatrix = CIRCLE;
        break;
      case OutlineFilter::Matrix::Square:
        commonMatrix = SQUARE;
        break;
      case OutlineFilter::Matrix::Horizontal:
        commonMatrix = HORZ;
        break;
      case OutlineFilter::Matrix::Vertical:
        commonMatrix = VERT;
        break;
    }
    ...
    for (int i = 0; i < 9; ++i) {
      m_panel.outlineMatrix()->getItem(i)->setIcon(
  ...
  void onMatrixTypeChange()
  {
    ...
    switch (m_panel.outlineType()->selectedItem()) {
      case CIRCLE:
        matrix = OutlineFilter::Matrix::Circle;
        break;
      case SQUARE:
        matrix = OutlineFilter::Matrix::Square;
        break;
      case HORZ:
        matrix = OutlineFilter::Matrix::Horizontal;
        break;
      case VERT:
        matrix = OutlineFilter::Matrix::Vertical;
        break;
    }
...
void OutlineCommand::onExecute(Context* context)
{
  ...
  if (ui) {
    filter.place((OutlineFilter::Place)get_config_int(
      ConfigSection, "Place", int(OutlineFilter::Place::Outside)));
    filter.matrix((OutlineFilter::Matrix)get_config_int(
      ConfigSection, "Matrix", int(OutlineFilter::Matrix::Circle)));
    filter.color(ColorBar::instance()->getFgColor());

    DocumentPreferences& docPref =
      Preferences::instance().document(site.document());
  ...
  filterMgr.setTarget(site.sprite()->pixelFormat() == IMAGE_INDEXED ?
                        ...
                        TARGET_RED_CHANNEL | TARGET_GREEN_CHANNEL |
