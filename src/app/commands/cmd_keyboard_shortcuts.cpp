...
public:
  HeaderSplitter()
    : Splitter(Splitter::ByPixel, HORIZONTAL)
  {
  }
...
private:
  ...
  void onAddAccel()
  {
    ...
    SelectAccelerator window(
      accel, m_key ? m_key->keycontext() : KeyContext::Any, m_keys);
    window.openWindowInForeground();
    ...
    if ((window.isModified()) ||
      ...
      if (!m_key) {
        ...
        m_key = m_keys.command(m_menuitem->getCommandId().c_str(),
  ...
  void onPaint(PaintEvent& ev) override
  {
    ...
    if (m_key && !m_key->accels().empty()) {
      ...
      IntersectClip clip(
        g,
        gfx::Rect(
          keyXPos, y, contextXPos - keyXPos, dh * m_key->accels().size()));
      if (clip) {
        int i = 0;
      ...
      case kMouseMoveMessage: {
        ...
        for (int i = 0; i < maxi; ++i, y += dh) {
          int w = Graphics::measureUITextLength(
            (accels && i < (int)accels->size() ?
               getAccelText((*accels)[i]).c_str() :
               ""),
            font());
          ...
          if (accels && i < (int)accels->size() &&
            ...
            if (m_hotAccel != i) {
              ...
              m_changeConn =
                m_changeButton->Click.connect([this, i] { onChangeAccel(i); });
              ...
              m_deleteConn =
                m_deleteButton->Click.connect([this, i] { onDeleteAccel(i); });
          ...
          if (i == 0 && !m_addButton &&
            ...
            itemBounds.w =
              8 * guiscale() + Graphics::measureUITextLength("Add", font());
            itemBounds.x -= itemBounds.w + 2 * guiscale();
...
public:
    ...
    , m_curSection(curSection)
  {
    ...
    wheelBehavior()->setSelectedItem(m_keys.hasMouseWheelCustomization() ? 1 :
                                                                           0);
...
private:
  ...
  void fillAllLists()
  {
...

    {
      ...
      KeyItemBase* listItem =
        new KeyItemBase(Strings::palette_popup_menu_title());
    ...
    for (const KeyPtr& key : m_keys) {
      if (key->type() == KeyType::Tool || key->type() == KeyType::Quicktool ||
          ...
          key->type() == KeyType::DragAction || key->isListed()) {
        continue;
      }
      ...
      switch (key->keycontext()) {
        ...
        case KeyContext::ShapeTool:
          text = convertKeyContextToUserFriendlyString(key->keycontext()) +
                 ": " + text;
          break;
      }
      KeyItem* keyItem =
        new KeyItem(m_keys, m_menuKeys, text, key, nullptr, 0, &m_headerItem);
  ...
  {
    slideZoom()->setText(isDefaultWheelBehavior() ?
  ...
  void onImport()
  {
    ...
    if (!app::show_file_selector(
          Strings::keyboard_shortcuts_import_keyboard_sc(),
          "",
          exts,
  ...
  void onExport()
  {
    ...
    if (!app::show_file_selector(
          Strings::keyboard_shortcuts_export_keyboard_sc(),
          "",
          exts,
  ...
  DragVector dirToAngle(Dir dir)
  {
    switch (dir) {
      case Dir::NW:
        return DragVector(-1, +1);
      case Dir::N:
        return DragVector(0, +1);
      case Dir::NE:
        return DragVector(+1, +1);
      case Dir::W:
        return DragVector(-1, 0);
      case Dir::C:
        return DragVector(0, 0);
      case Dir::E:
        return DragVector(+1, 0);
      case Dir::SW:
        return DragVector(-1, -1);
  ...
  for (auto child : menu->children()) {
    if (AppMenuItem* menuItem = dynamic_cast<AppMenuItem*>(child)) {
      ...
      if (menuItem->getCommand()) {
        menuKeys[menuItem] =
          keys.command(menuItem->getCommandId().c_str(), menuItem->getParams());
      }
