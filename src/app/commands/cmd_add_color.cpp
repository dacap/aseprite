...
void AddColorCommand::onExecute(Context* ctx)
{
  ...
  try {
    ...
    color_t color = doc::rgba(appColor.getRed(),
                              appColor.getGreen(),
                              appColor.getBlue(),
                              appColor.getAlpha());
...
std::string AddColorCommand::onGetFriendlyName() const
{
  ...
  switch (params().source()) {
    case AddColorSource::Fg:
      source = Strings::commands_AddColor_Foreground();
      break;
    case AddColorSource::Bg:
      source = Strings::commands_AddColor_Background();
      break;
    case AddColorSource::Color:
      source = Strings::commands_AddColor_Specific();
      break;
  }
