...
protected:
  ...
  gfx::Rect getRectFromEntries()
  {
    ...
    return gfx::Rect(
      x()->textInt(), y()->textInt(), std::max<int>(1, w), std::max<int>(1, h));
  }
...
private:
  void selectActiveDocument()
  {
    ...
    if (m_document) {
      ...
      paddingEnabled()->setSelected(
        m_docPref->importSpriteSheet.paddingEnabled());
  ...
  void updateGridState()
  {
    ...
    int flags =
      (int)static_cast<SelectBoxState*>(m_editorState.get())->getFlags();
    flags = flags & ~((int)SelectBoxState::Flags::HGrid |
                      (int)SelectBoxState::Flags::VGrid);
...
class ImportSpriteSheetCommand
  : public CommandWithNewParams<ImportSpriteSheetParams> {
public:
...
void ImportSpriteSheetCommand::onExecute(Context* context)
{
  ...
  try {
    ...
    for (const auto& tileRect : tileRects) {
      ...
      render.renderSprite(
        resultImage.get(), sprite, currentFrame, gfx::Clip(0, 0, tileRect));
