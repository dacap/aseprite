...
{
  switch (toGray) {
    case gen::ToGrayAlgorithm::LUMA:
      return &rgba_to_graya_using_luma;
    case gen::ToGrayAlgorithm::HSV:
      return &rgba_to_graya_using_hsv;
    case gen::ToGrayAlgorithm::HSL:
      return &rgba_to_graya_using_hsl;
  }
  return nullptr;
}
...
private:
  void run(const Sprite* sprite,
           ...
           const bool newBlend)
  {
    doc::ImageRef tmp(Image::create(
      sprite->pixelFormat(), m_image->width(), m_image->height()));
...
public:
    ...
    : m_pixelFormat(pixelFormat)
  {
    ...
    switch (pixelFormat) {
      case IMAGE_RGB:
        toMode = Strings::commands_ChangePixelFormat_RGB();
        break;
      case IMAGE_GRAYSCALE:
        toMode = Strings::commands_ChangePixelFormat_Grayscale();
        break;
      case IMAGE_INDEXED:
        toMode = Strings::commands_ChangePixelFormat_Indexed();
        break;
    }
...
public:
    ...
    , m_imageJustCreated(true)
  {
    ...
    switch (from) {
      case IMAGE_RGB:
        fromMode = Strings::commands_ChangePixelFormat_RGB();
        break;
      case IMAGE_GRAYSCALE:
        fromMode = Strings::commands_ChangePixelFormat_Grayscale();
        break;
      case IMAGE_INDEXED:
        fromMode = Strings::commands_ChangePixelFormat_Indexed();
        break;
    }
    ...
    if (from != IMAGE_INDEXED) {
      ...
      m_ditheringSelector =
        new DitheringSelector(DitheringSelector::SelectBoth);
      ...
      m_bestFitCriteriaSelector->Change.connect(
        [this] { onIndexParamChange(); });
    ...
    colorModeView()->setMinSize(colorModeView()->sizeHint() +
                                colorMode()->sizeHint());
...
private:
  ...
  void onChangeColorMode()
  {
    ...
    if (
      item ==
      m_selectedItem)  // Avoid restarting the conversion process for the same option
      return;
    m_selectedItem = item;
    ...
    m_image.reset(Image::create(
      dstPixelFormat, visibleBounds.w, visibleBounds.h, m_imageBuffer));
      ...
      case IMAGE_INDEXED:
        switch (m_dithering.algorithm()) {
          ...
          case render::DitheringAlgorithm::Ordered:
            conversion =
              Strings::commands_ChangePixelFormat_Indexed_OrderedDithering();
            break;
          case render::DitheringAlgorithm::Old:
            conversion =
              Strings::commands_ChangePixelFormat_Indexed_OldDithering();
            break;
          case render::DitheringAlgorithm::ErrorDiffusion:
            conversion =
              Strings::commands_ChangePixelFormat_Indexed_ErrorDiffusion();
            break;
        }
        break;
    }
