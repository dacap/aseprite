...
class CssFormat : public FileFormat {
  ...
  {
    return FILE_SUPPORT_SAVE | FILE_SUPPORT_RGB | FILE_SUPPORT_RGBA |
...
bool CssFormat::onSave(FileOp* fop)
{
  ...
  const auto css_options =
    std::static_pointer_cast<CssOptions>(fop->formatOptions());
  FileHandle handle(
  ...
  auto print_shadow_color = [f, css_options, print_color](int x,
                                                          int y,
                                                          int r,
                                                          int g,
                                                          int b,
  ...
  auto print_shadow_index = [f, css_options](
                              int x, int y, int i, bool comma = true) {
    fprintf(f, comma ? ",\n" : "\n");
    fprintf(
      f,
  ...
  if (css_options->generateHtml) {
    ...
    FileHandle handle(
      open_file_with_exception_sync_on_close(html_filepath, "wb"));
    FILE* h = handle.get();
    fprintf(h,
