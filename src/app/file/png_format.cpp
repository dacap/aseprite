...
bool PngFormat::onLoad(FileOp* fop)
{
  ...
  if (png_get_text(png, info, &text, &num_text) != 0) {
    for (int i = 0; i < num_text; i++) {
      ...
      if (src.text)
        entry.text = std::make_unique<base::buffer>(src.text, src.text + strlen(src.text) + 1);
      if (src.lang)
        entry.lang = std::make_unique<base::buffer>(src.lang, src.lang + strlen(src.lang) + 1);
      if (src.lang_key)
        entry.lang_key = std::make_unique<base::buffer>(src.lang_key, src.lang_key + strlen(src.lang_key) + 1);

      PNG_TRACE("PNG: Read text chunk '%s'\n", src.key);
  ...
  if (opts) {
    ...
    if (!text.empty()) {
      ...
      for (const auto& entry : text) {
        ...
        PNG_TRACE("PNG: Write text chunk '%s'\n", dst.key);
      }
