...
class FileOp {
public:
  static FileOp* createLoadDocumentOperation(
    Context* context,
  ...
  bool hasIncompatibilityError() const
  {
    return !m_incompatibilityError.empty();
  }
...
private:
  ...
  std::string
    m_dataFilename;  // File-name for a special XML .aseprite-data where extra sprite data can be stored
  FileOpROI m_roi;
