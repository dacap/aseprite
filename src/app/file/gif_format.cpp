...
private:
  ...
  void readImageDescRecord()
  {
    ...
    GIF_TRACE(
      "GIF: Frame[%d] transparentIndex=%d localMap=%d\n",
      (int)m_frameNum,
  ...
  {
    gfx::Clip clip(
      frameBounds.x, frameBounds.y, 0, 0, frameBounds.w, frameBounds.h);
    ...
    LockImageBits<IndexedTraits> dstBits(m_currentImage.get(),
                                         clip.dstBounds());
  ...
  void convertIndexedSpriteToRgb()
  {
    ...
    m_currentImage.reset(
      render::convert_pixel_format(m_currentImage.get(),
                                   NULL,
                                   IMAGE_RGB,
                                   render::Dithering(),
                                   nullptr,
  ...
  Remap m_remap;
  bool
    m_hasLocalColormaps;  // Indicates that this fila contains local colormaps
...
public:
  ...
  {
    const auto gifOptions =
      std::static_pointer_cast<GifOptions>(fop->formatOptions());
    ...
    for (int i = 0; i < 3; ++i)
      m_images[i].reset(
        Image::create((m_preservePaletteOrder) ? IMAGE_INDEXED : IMAGE_RGB,
                      m_spriteBounds.w,
                      m_spriteBounds.h));
  }
...
private:
                                              ...
                                              DisposalMethod& disposal)
  {
    if (gifFrame == 0) {
      ...
      if (!m_preservePaletteOrder) {
        ...
        const LockImageBits<RgbTraits> bits3(m_nextImage);
        m_deltaImage.reset(Image::create(
          PixelFormat::IMAGE_RGB, m_spriteBounds.w, m_spriteBounds.h));
        ...
        typename LockImageBits<RgbTraits>::const_iterator it1, it3, end1,
          deltaEnd;
  ...
  void writeLoopExtension()
  {
    ...
    if (EGifPutExtensionFirst(
          m_gifFile, APPLICATION_EXT_FUNC_CODE, 11, extension_bytes) ==
        GIF_ERROR)
    ...
    extension_bytes[1] = (m_loop & 0xff);
    extension_bytes[2] = (m_loop >> 8) & 0xff;
    if (EGifPutExtensionNext(
          m_gifFile, APPLICATION_EXT_FUNC_CODE, 3, extension_bytes) ==
        GIF_ERROR)
      throw Exception("Error writing GIF graphics extension record.\n");

    if (EGifPutExtensionLast(m_gifFile, APPLICATION_EXT_FUNC_CODE, 0, NULL) ==
        GIF_ERROR)
      throw Exception("Error writing GIF graphics extension record.\n");
  #endif
  }
                      ...
                      const bool fixDuration)
  {
    ...
    if (EGifPutExtension(
          m_gifFile, GRAPHICS_EXT_FUNC_CODE, 4, extension_bytes) == GIF_ERROR)
      throw Exception(
                  ...
                  const bool fixDuration)
  {
    ...
    ImageRef frameImage(Image::create(
      IMAGE_INDEXED, frameBounds.w, frameBounds.h, m_frameImageBuf));
  ...
  if (fop->context() && fop->context()->isUIAvailable()) {
    try {
      ...
      if (pref.gif.showAlert()) {
        ...
        if (fop->document()->sprite()->pixelFormat() ==
        ...
        if (win.closer() == win.ok()) {
          ...
          pref.gif.preservePaletteOrder(
            win.preservePaletteOrder()->isSelected());
