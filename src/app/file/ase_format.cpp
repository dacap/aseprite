...
public:
  ImageScanlines(const Image* image)
    : m_image(image)
  {
  }
  gfx::Size getImageSize() const override
  {
    return gfx::Size(m_image->width(), m_image->height());
  }
...
public:
  TilesetScanlines(const Tileset* tileset)
    : m_tileset(tileset)
  {
  }
  gfx::Size getImageSize() const override
...
static void ase_file_write_header_filesize(FILE* f,
                                           dio::AsepriteHeader* header);

static void ase_file_prepare_frame_header(
  FILE* f, dio::AsepriteFrameHeader* frame_header);
static void ase_file_write_frame_header(FILE* f,
...
static void ase_file_write_cel_chunk(FILE* f,
                                     ...
                                     const Sprite* sprite,
                                     const frame_t firstFrame);
static void ase_file_write_cel_extra_chunk(
  FILE* f, dio::AsepriteFrameHeader* frame_header, const Cel* cel);
...
static void ase_file_write_slice_chunks(
  FILE* f,
  FileOp* fop,
  dio::AsepriteFrameHeader* frame_header,
  const dio::AsepriteExternalFiles& ext_files,
...
static void ase_file_write_slice_chunk(FILE* f,
                                       ...
                                       const frame_t fromFrame,
                                       const frame_t toFrame);
static void ase_file_write_user_data_chunk(
  FILE* f,
  FileOp* fop,
  dio::AsepriteFrameHeader* frame_header,
  const dio::AsepriteExternalFiles& ext_files,
...
static void ase_file_write_external_files_chunk(
  ...
  const Sprite* sprite);
static void ase_file_write_tileset_chunks(
  FILE* f,
  FileOp* fop,
  dio::AsepriteFrameHeader* frame_header,
  const dio::AsepriteExternalFiles& ext_files,
  const Tilesets* tilesets);
static void ase_file_write_tileset_chunk(
  FILE* f,
  FileOp* fop,
  dio::AsepriteFrameHeader* frame_header,
  const dio::AsepriteExternalFiles& ext_files,
...
static void ase_file_write_properties_maps(
  FILE* f,
  FileOp* fop,
  const dio::AsepriteExternalFiles& ext_files,
  size_t nmaps,
...
class ChunkWriter {
public:
  ChunkWriter(FILE* f, dio::AsepriteFrameHeader* frame_header, int type)
    : m_file(f)
  {
...
class AseFormat : public FileFormat {
  ...
  int onGetFlags() const override
  {
    return FILE_SUPPORT_LOAD | FILE_SUPPORT_SAVE | FILE_SUPPORT_RGB |
           ...
           FILE_SUPPORT_PALETTES | FILE_SUPPORT_TAGS |
...
bool AseFormat::onPostLoad(FileOp* fop)
{
  ...
  if (flat && ase_has_groups(group)) {
    if (
      fop->context() && fop->context()->isUIAvailable() &&
...
bool AseFormat::onSave(FileOp* fop)
{
  ...
  // Write the header
  dio::AsepriteHeader header;
  ase_file_prepare_header(
    f, &header, sprite, fop->roi().fromFrame(), fop->roi().frames());
  ...
  for (frame_t frame : fop->roi().framesSequence()) {
    ...
    if (outputFrame == 0) {
      ...
      ase_file_write_external_files_chunk(
        f, fop, &frame_header, ext_files, sprite);
    ...
    if (frame == fop->roi().fromFrame()) {
      ...
      if (!sprite->userData().isEmpty())
        ase_file_write_user_data_chunk(
          f, fop, &frame_header, ext_files, &sprite->userData());

      // Write tilesets
      ase_file_write_tileset_chunks(
        f, fop, &frame_header, ext_files, sprite->tilesets());
      ...
      if (sprite->tags().size() > 0) {
        ...
        for (doc::Tag* tag : sprite->tags()) {
          ase_file_write_user_data_chunk(
            f, fop, &frame_header, ext_files, &(tag->userData()));
        }
      }
    ...
    // Write cel chunks
    ase_file_write_cels(
      f, fop, &frame_header, ext_files, sprite, sprite->root(), 0, frame);
                                  ...
                                  int child_index)
{
  ...
  if (!layer->userData().isEmpty())
    ase_file_write_user_data_chunk(
      f, fop, frame_header, ext_files, &layer->userData());

  if (layer->isGroup()) {
    for (const Layer* child : static_cast<const LayerGroup*>(layer)->layers())
      ase_file_write_layers(
        f, fop, frame_header, ext_files, child, child_index + 1);
  }
}
                                   ...
                                   const frame_t frame)
{
  if (layer->isImage()) {
    ...
    if (cel) {
      ...
      if (!cel->link() && !cel->data()->userData().isEmpty()) {
        ase_file_write_user_data_chunk(
          f, fop, frame_header, ext_files, &cel->data()->userData());
      }
    }
  }
  ...
  if (layer->isGroup()) {
    for (const Layer* child : static_cast<const LayerGroup*>(layer)->layers()) {
      layer_index = ase_file_write_cels(
        f, fop, frame_header, ext_files, sprite, child, layer_index, frame);
    }
  }
                                       ...
                                       int child_level)
{
  ...
  fputw(layer->isImage() ?
        ...
        f);
  fputc(layer->isImage() ?
          (int)static_cast<const LayerImage*>(layer)->opacity() :
          0,
        f);
...
public:
  ...
  void write_scanline(GrayscaleTraits::address_t address,
                      int w,
                      uint8_t* buffer)
  {
    for (int x = 0; x < w; ++x, ++address) {
                                      ...
                                      const frame_t toFrame)
{
  ...
  for (const Tag* tag : *tags) {
    ...
    frame_t from =
      std::clamp(tag->fromFrame() - fromFrame, 0, toFrame - fromFrame);
    frame_t to =
      std::clamp(tag->toFrame() - fromFrame, from, toFrame - fromFrame);
  ...
  const UserData* userData)
{
  ...
  if (flags & ASE_USER_DATA_FLAG_HAS_PROPERTIES) {
    ase_file_write_properties_maps(
      f, fop, ext_files, nmaps, userData->propertiesMaps());
  }
}
  ...
  const frame_t toFrame)
{
  for (Slice* slice : slices) {
    ...
    if (!slice->userData().isEmpty())
      ase_file_write_user_data_chunk(
        f, fop, frame_header, ext_files, &slice->userData());
  }
}
  ...
  const Sprite* sprite)
{
  ...
  std::deque<Layer*> layers(sprite->root()->layers().begin(),
                            sprite->root()->layers().end());
  while (!layers.empty()) {
  ...
  const Tilesets* tilesets)
{
  ...
  for (const Tileset* tileset : *tilesets) {
    if (tileset) {
      ase_file_write_tileset_chunk(
        f, fop, frame_header, ext_files, tileset, si);

      ase_file_write_user_data_chunk(
        f, fop, frame_header, ext_files, &tileset->userData());

      // Write tile UserData
      for (tile_index i = 0; i < tileset->size(); ++i) {
        UserData tileData = tileset->getTileData(i);
        ase_file_write_user_data_chunk(
          f, fop, frame_header, ext_files, &tileData);
      }
    }
    ++si;
  }
  ...
  const tileset_index si)
{
  ...
  if (flags & ASE_TILESET_FLAG_EXTERNAL_FILE) {
    uint32_t file_id = 0;
    if (ext_files.getIDByFilename(
          ASE_EXTERNAL_FILE_TILESET, tileset->externalFilename(), file_id)) {
      fputl(file_id, f);
  ...
  if (flags & ASE_TILESET_FLAG_EMBEDDED) {
    ...
    // Compress and save the tileset now
      ...
      write_compressed_image(
        f, &gen, tileset->sprite()->pixelFormat(), compressedDataPtr);
  ...
  const doc::UserData::PropertiesMaps& propertiesMaps)
{
  ...
  for (const auto& propertiesMap : propertiesMaps) {
    ...
    if (!extensionKey.empty() &&
        !ext_files.getIDByFilename(
          ASE_EXTERNAL_FILE_EXTENSION, extensionKey, extensionId)) {
