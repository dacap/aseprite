...
void update_xml_part_from_slice_key(const doc::SliceKey* key,
                                    XMLElement* xmlPart)
{
...
void update_xml_slice(const doc::Slice* slice, XMLElement* xmlSlice)
{
  ...
  xmlSlice->SetAttribute("color",
                         color_to_hex(slice->userData().color()).c_str());
                             ...
                             app::Color& defaultSliceColor)
{
  ...
  if (xmlSlices && xmlSlices->Attribute("theme")) {
    ...
    for (XMLElement* xmlPart = themeHandle.FirstChildElement("theme")
      ...
      if (xmlPart->Attribute("w1")) {
        int w1 = xmlPart->Attribute("w1") ?
                   std::strtol(xmlPart->Attribute("w1"), NULL, 10) :
                   0;
        int w2 = xmlPart->Attribute("w2") ?
                   std::strtol(xmlPart->Attribute("w2"), NULL, 10) :
                   0;
        int w3 = xmlPart->Attribute("w3") ?
                   std::strtol(xmlPart->Attribute("w3"), NULL, 10) :
                   0;
        int h1 = xmlPart->Attribute("h1") ?
                   std::strtol(xmlPart->Attribute("h1"), NULL, 10) :
                   0;
        int h2 = xmlPart->Attribute("h2") ?
                   std::strtol(xmlPart->Attribute("h2"), NULL, 10) :
                   0;
        int h3 = xmlPart->Attribute("h3") ?
                   std::strtol(xmlPart->Attribute("h3"), NULL, 10) :
                   0;
        ...
        int w = xmlPart->Attribute("w") ?
                  std::strtol(xmlPart->Attribute("w"), NULL, 10) :
                  0;
        int h = xmlPart->Attribute("h") ?
                  std::strtol(xmlPart->Attribute("h"), NULL, 10) :
                  0;
        key.setBounds(gfx::Rect(x, y, w, h));
      }

      if (xmlPart->Attribute("focusx")) {
        int x = xmlPart->Attribute("focusx") ?
                  std::strtol(xmlPart->Attribute("focusx"), NULL, 10) :
                  0;
        int y = xmlPart->Attribute("focusy") ?
                  std::strtol(xmlPart->Attribute("focusy"), NULL, 10) :
                  0;
  ...
  // Load slices from <slice> elements
    ...
    for (XMLElement* xmlSlice =
           (xmlSlices->FirstChildElement("slice") ?
      ...
      for (XMLElement* xmlKey =
             (xmlSlice->FirstChildElement("key") ?
                xmlSlice->FirstChildElement("key")->ToElement() :
                nullptr);
           xmlKey;
        ...
        doc::SliceKey key;
        doc::frame_t frame =
          std::strtol(xmlKey->Attribute("frame"), nullptr, 10);
                             ...
                             const doc::Document* doc)
{
  ...
  if (xmlSlices && xmlSlices->Attribute("theme")) {
    ...
    XMLHandle themeHandle(xmlThemeDoc.get());
    XMLElement* xmlParts = themeHandle.FirstChildElement("theme")
                             .FirstChildElement("parts")
                             .ToElement();
