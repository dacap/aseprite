...
void FlattenLayers::onExecute()
{
...

  {
    ...
    for (frame_t frame(0); frame < sprite->totalFrames(); ++frame) {
      ...
      const bool shrink = doc::algorithm::shrink_bounds(
        image.get(), image->maskColor(), nullptr, image->bounds(), bounds);
      ...
      if (cel) {
        ...
        if (!newFlatLayer) {
          ...
          executeAndAdd(
            new cmd::SetCelPosition(cel, area.x + bounds.x, area.y + bounds.y));
        }
  ...
  if (newFlatLayer) {
    executeAndAdd(
      new cmd::AddLayer(list.front()->parent(), flatLayer, list.front()));
  }
  // Reset layer properties when flattening in-place
    ...
    if (flatLayer->blendMode() != doc::BlendMode::NORMAL)
      executeAndAdd(
        new cmd::SetLayerBlendMode(flatLayer, doc::BlendMode::NORMAL));
  }
