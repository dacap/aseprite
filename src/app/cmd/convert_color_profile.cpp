...
static doc::ImageRef convert_image_color_space(
  ...
  if (sprite->pixelFormat() != doc::IMAGE_INDEXED) {
    for (Cel* cel : sprite->uniqueCels()) {
      ...
      if (old_image.get()->pixelFormat() != IMAGE_TILEMAP) {
        ImageRef new_image =
          convert_image_color_space(old_image.get(), newCS, conversion.get());
    ...
    if (sprite->pixelFormat() != doc::IMAGE_GRAYSCALE) {
      for (auto& pal : sprite->getPalettes()) {
        ...
        for (int i = 0; i < pal->size(); ++i) {
          ...
          conversion->convertRgba(
            (uint32_t*)&newCol, (const uint32_t*)&oldCol, 1);
  ...
  if (conversion) {
    switch (image->pixelFormat()) {
      ...
      case doc::IMAGE_GRAYSCALE: {
        ImageRef newImage =
          convert_image_color_space(image, newCS, conversion.get());
...
ConvertColorProfile::ConvertColorProfile(doc::Sprite* sprite,
                                         const gfx::ColorSpaceRef& newCS)
  : WithSprite(sprite)
{
  os::System* system = os::instance();
