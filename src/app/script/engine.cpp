...
int dofile(lua_State* L)
{
  ...
  if (!base::is_file(fname) && !current_script_dirs.empty()) {
    ...
    std::string altFname =
      base::join_path(base::get_file_path(current_script_dirs.top()), fname);
...
int loadfile(lua_State* L)
{
  ...
  if (auto app = App::instance(); app && app->isGui() && !lua_isstring(L, 1)) {
    return luaL_error(
      L, "loadfile() for stdin cannot be used running in GUI mode");
  }
  ...
  , m_printLastResult(false)
{
  ...
  setfield_integer(
    L, "ALPHA_COMPOSITING", app::tools::InkType::ALPHA_COMPOSITING);
  ...
  setfield_integer(
    L, "RGB", TARGET_RED_CHANNEL | TARGET_GREEN_CHANNEL | TARGET_BLUE_CHANNEL);
  setfield_integer(L,
                   "RGBA",
...
void Engine::startDebugger(DebuggerDelegate* debuggerDelegate)
{
  ...
  lua_sethook(
    L, hook, LUA_MASKCALL | LUA_MASKRET | LUA_MASKLINE | LUA_MASKCOUNT, 1);
}
