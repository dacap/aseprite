...
struct Dialog {
  ...
  gfx::Rect getWindowBounds() const
  {
    ...
    if (window.ownDisplay()) {
      ...
      const gfx::Point dialogOrigin =
        window.display()->nativeWindow()->contentRect().origin();
      const gfx::Point mainOrigin =
  ...
  if (lua_istable(L, 1)) {
    ...
    if (type == LUA_TFUNCTION) {
      Dialog_connect_signal(
        L, -2, dlg->window.Close, [](lua_State*, CloseEvent&) {
          // Do nothing
        });
    }
    lua_pop(L, 1);
...
int Dialog_showMenu(lua_State* L)
{
  ...
  gfx::Point pt = dlg->parentDisplay()->nativeWindow()->pointFromScreen(
...
int Dialog_add_widget(lua_State* L, Widget* widget)
{
  ...
  if (label || !dlg->hbox) {
    if (label) {
      ...
      if (widget->type() != Tabs::Type()) {
        dlg->currentGrid->addChildInCell(
          new ui::HBox, 1, 1, ui::LEFT | ui::TOP);
      }
    }
    ...
    dlg->currentGrid->addChildInCell(
      hbox, hspan, 1, ui::HORIZONTAL | (vexpand ? ui::VERTICAL : ui::TOP));
...
int Dialog_color(lua_State* L)
{
  ...
  auto widget = new ColorButton(
    color, app_get_current_pixel_format(), ColorButtonOptions());

  if (lua_istable(L, 2)) {
    int type = lua_getfield(L, 2, "onchange");
    if (type == LUA_TFUNCTION) {
      Dialog_connect_signal(
        L, 1, widget->Change, [](lua_State* L, const app::Color& color) {
...
int Dialog_file(lua_State* L)
{
  ...
  widget->SelectFile.connect([=]() -> std::string {
    ...
    if (app::show_file_selector(
          title, widget->filename(), exts, dlgType, newfilename))
...
static void fill_message_values(lua_State* L, const ui::Message* msg)
{
  ...
  lua_pushboolean(
    L, msg->modifiers() & (ui::kKeyCmdModifier | ui::kKeyWinModifier));
...
static void fill_keymessage_values(lua_State* L, const ui::KeyMessage* msg)
{
  ...
  lua_setfield(
    L,
    -2,
    "repeat");  // Only for backward compatibility, remove this in a future
...
int Dialog_canvas(lua_State* L)
{
  ...
  if (lua_istable(L, 2)) {
    ...
    if (lua_istable(L, 2)) {
      ...
      if (type == LUA_TFUNCTION) {
        Dialog_connect_signal(
          L, 1, widget->Paint, [](lua_State* L, GraphicsContext& gc) {
      ...
      if (type == LUA_TFUNCTION) {
        Dialog_connect_signal(
          L, 1, widget->MouseMove, fill_mousemessage_values);
      }
      lua_pop(L, 1);
      ...
      if (type == LUA_TFUNCTION) {
        Dialog_connect_signal(
          L, 1, widget->MouseDown, fill_mousemessage_values);
      }
      lua_pop(L, 1);
      ...
      if (type == LUA_TFUNCTION) {
        Dialog_connect_signal(
          L, 1, widget->DoubleClick, fill_mousemessage_values);
      }
      lua_pop(L, 1);
      ...
      if (type == LUA_TFUNCTION) {
        Dialog_connect_signal(
          L, 1, widget->TouchMagnify, fill_touchmessage_values);
      }
      lua_pop(L, 1);
    }
...
int Dialog_endtabs(lua_State* L)
{
  ...
  if (lua_istable(L, 2)) {
    ...
    switch (type) {
      ...
      case LUA_TNUMBER:
        selectedTab =
          std::clamp<int>(lua_tointeger(L, -1), 1, dlg->wipTab->size()) - 1;
        break;
    }
    lua_pop(L, 1);
...
int Dialog_modify(lua_State* L)
{
  ...
  if (lua_istable(L, 2)) {
    ...
    if (!widget)
      return luaL_error(
        L, "Given id=\"%s\" in Dialog:modify{} not found in dialog", id);
  ...
  for (const auto& kv : dlg->dataWidgets) {
    ...
    switch (widget->type()) {
      ...
      default:
        ...
        else if (auto filenameField =
                   dynamic_cast<const FilenameField*>(widget)) {
...
const luaL_Reg Dialog_methods[] = {
  { "__gc", Dialog_gc },           { "show", Dialog_show },
  { "showMenu", Dialog_showMenu }, { "close", Dialog_close },
  { "newrow", Dialog_newrow },     { "separator", Dialog_separator },
  { "label", Dialog_label },       { "button", Dialog_button },
  { "check", Dialog_check },       { "radio", Dialog_radio },
  { "menuItem", Dialog_menuItem }, { "entry", Dialog_entry },
  { "number", Dialog_number },     { "slider", Dialog_slider },
  { "combobox", Dialog_combobox }, { "color", Dialog_color },
  { "shades", Dialog_shades },     { "file", Dialog_file },
  { "canvas", Dialog_canvas },     { "tab", Dialog_tab },
  { "endtabs", Dialog_endtabs },   { "modify", Dialog_modify },
  { "repaint", Dialog_repaint },   { nullptr, nullptr }
};
