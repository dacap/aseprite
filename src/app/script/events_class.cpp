...
protected:
  void call(EventType eventType,
            const std::initializer_list<std::pair<const std::string, std::any>>&
              args = {})
  {
    if (eventType >= m_listeners.size())
      return;
  ...
  void onAddFirstListener(EventType eventType) override
  {
    ...
    switch (eventType) {
      ...
      case BgColorChange:
        ...
        break;
      case BeforeCommand:
        m_beforeCmdConn = ctx->BeforeCommandExecution.connect(
          &AppEvents::onBeforeCommand, this);
        break;
      case AfterCommand:
        m_afterCmdConn =
          ctx->AfterCommandExecution.connect(&AppEvents::onAfterCommand, this);
        break;
    }
  }
...
{
  call(AfterCommand,
       { { "name", ev.command()->id() }, { "params", ev.params() } });
}
...
private:
  ...
  {
    call(Resize, { { "width", ev.bounds().w }, { "height", ev.bounds().h } });
  }
...
int Events_off(lua_State* L)
{
  ...
  // Remove by function reference
    ...
    return luaL_error(L,
                      "first argument must be a function or a EventListener");
  }
...
const luaL_Reg Events_methods[] = { { "on", Events_on },
                                    { "off", Events_off },
                                    { nullptr, nullptr } };
