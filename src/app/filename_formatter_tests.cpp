...
TEST(FilenameFormatter, Basic)
{
  ...
  EXPECT_EQ(
    "file.png",
    filename_formatter("{name}", FilenameInfo().filename("C:/temp/file.png")));
...
TEST(FilenameFormatter, GetFrameInfo)
{
  ...
  EXPECT_EQ(false,
            get_frame_info_from_filename_format("hi.png", nullptr, nullptr));

  frameBase = width = -1;
  EXPECT_EQ(
    true,
    get_frame_info_from_filename_format("hi{frame}.png", &frameBase, &width));
  ...
  EXPECT_EQ(
    true,
    get_frame_info_from_filename_format("hi{frame1}.png", &frameBase, &width));
  EXPECT_EQ(1, frameBase);
  EXPECT_EQ(1, width);

  frameBase = width = -1;
  EXPECT_EQ(true,
            get_frame_info_from_filename_format(
              "hi{frame032}.png", &frameBase, &width));
...
TEST(FilenameFormatter, DefaultFormat)
{
  ...
  fn = "/path/hello.png";
  EXPECT_EQ("{title}.{extension}",
            get_default_filename_format(fn, false, false, false, false));
...
TEST(FilenameFormatter, DetectFrame)
{
  ...
  fn = "/path/hello.png";
  EXPECT_EQ("/path/hello{frame1}.png",
            replace_frame_number_with_frame_format(fn));

  fn = "/path/hello0.png";
  EXPECT_EQ("/path/hello{frame0}.png",
            replace_frame_number_with_frame_format(fn));
  ...
  EXPECT_EQ("/path/hello{frame002}.png",
            replace_frame_number_with_frame_format(fn));
}
