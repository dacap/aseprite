                                   ...
                                   const Transformation& transform)
{
  ...
  if (visiblePivot(angle)) {
    ...
    g->drawRgbaSurface(
      part, pivotBounds.x - origin.x, pivotBounds.y - origin.y);
  }
}
                                         ...
                                         const Transformation& transform)
{
  ...
  for (size_t c = 0; c < HANDLES; ++c) {
    ...
    adjustHandle(
      u, v, part->width(), part->height(), angle + handles_info[c].angle);
...
gfx::Rect TransformHandles::getPivotHandleBounds(
  Editor* editor,
  const Transformation& transform,
...
bool TransformHandles::inHandle(
  const gfx::Point& pt, int x, int y, int gfx_w, int gfx_h, double angle)
{
...
void TransformHandles::drawHandle(
  Editor* editor, Graphics* g, int x, int y, double angle)
{
  auto theme = SkinTheme::get(editor);
  os::Surface* part = theme->parts.transformationHandle()->bitmap(0);
...
void TransformHandles::adjustHandle(
  int& x, int& y, int handle_w, int handle_h, double angle)
{
