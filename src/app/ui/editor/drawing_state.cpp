                                ...
                                const tools::Pointer& pointer)
{
  ...
  Layer* previewLayer =
    (m_toolLoop->getInk()->isSelection() || m_toolLoop->getInk()->isSlice() ?
       nullptr :
       m_toolLoop->getLayer());
  ...
  editor->renderEngine().setPreviewImage(
    ...
    (previewLayer && previewLayer->isImage() ?
       ...
       doc::BlendMode::
         NEG_BW));  // To preview the selection ink we use the negative black & white blender
  ...
  m_mouseDownPos =
    (msg ? msg->position() : editor->editorToScreen(pointer.point()));
...
bool DrawingState::onMouseUp(Editor* editor, MouseMessage* msg)
{
  ...
  if (!m_toolLoop->getInk()->isSelection() ||
      ...
      (editor->getToolLoopModifiers() !=
         tools::ToolLoopModifiers::kReplaceSelection &&
       editor->getToolLoopModifiers() !=
         tools::ToolLoopModifiers::kIntersectSelection)) {
...
bool DrawingState::onKeyDown(Editor* editor, KeyMessage* msg)
{
  ...
  Params params;
  if (KeyboardShortcuts::instance()->getCommandFromKeyMessage(
        msg, &command, &params)) {
...
bool DrawingState::onScrollChange(Editor* editor)
{
  if (m_processScrollChange) {
    ...
    // Update velocity sensor.
    m_velocity.updateWithDisplayPoint(
      mousePos);  // TODO add scroll as velocity?
...
bool DrawingState::canInterpretMouseMovementAsJustOneClick()
{
  ...
  return !m_mouseMoveReceived ||
         (m_mouseMaxDelta.x < 4 && m_mouseMaxDelta.y < 4 &&
