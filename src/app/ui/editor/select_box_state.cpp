...
SelectBoxState::SelectBoxState(SelectBoxDelegate* delegate,
                               const gfx::Rect& rc,
                               Flags flags)
  : m_delegate(delegate)
...
void SelectBoxState::setPaddingBounds(const gfx::Size& padding)
{
  ASSERT(hasFlag(Flags::PaddingRulers));
  m_rulers[PH] =
    Ruler(HORIZONTAL | BOTTOM, m_rulers[H2].position() + padding.h);
...
bool SelectBoxState::onMouseDown(Editor* editor, MouseMessage* msg)
{
  if (msg->left() || msg->right()) {
    m_startingPos =
      editor->screenToEditor(msg->position()) - editor->mainTilePosition();
    ...
    if (hasFlag(Flags::PaddingRulers)) {
      ...
      if (rc.x > vp.x)
        render->fillRect(dark,
                         gfx::Rect(vp.x, rc.y, rc.x - vp.x, vp.y2() - rc.y));
      if (hasFlag(Flags::VGrid)) {
        // Bottom band
        if (sp.y2() < vp.y2())
          render->fillRect(
            dark, gfx::Rect(rc.x, sp.y2(), vp.x2() - rc.x, vp.y2() - sp.y2()));
        // Right band
        if (hasFlag(Flags::HGrid)) {
          ...
          if (sp.x2() < vp.x2())
            render->fillRect(
              dark,
              gfx::Rect(sp.x2(), rc.y, vp.x2() - sp.x2(), sp.y2() - rc.y));
        }
        else {
          ...
          if (rc.x2() < vp.x2())
            render->fillRect(
              dark,
              gfx::Rect(rc.x2(), rc.y, vp.x2() - rc.x2(), sp.y2() - rc.y));
        }
      }
      else {
        if (hasFlag(Flags::HGrid)) {
          ...
          if (rc.y2() < vp.y2())
            render->fillRect(
              dark,
              gfx::Rect(rc.x, rc.y2(), vp.x2() - rc.x, vp.y2() - rc.y2()));
          // Right band
          if (sp.x2() < vp.x2())
            render->fillRect(dark,
                             gfx::Rect(sp.x2(), rc.y, vp.x2() - sp.x2(), rc.h));
        }
      }
        ...
        if (rc.w > 0) {
          for (int x = rc.x + rc.w; x <= sp.x + sp.w; x += (rc.w + padding.w)) {
            if (x + padding.w > sp.x2())
              render->fillRect(dark,
                               gfx::Rect(x, rc.y, sp.x2() - x, sp.h - rc.y));
            else
              render->fillRect(dark,
                               gfx::Rect(x, rc.y, padding.w, sp.h - rc.y));
          ...
          for (int y = rc.y + rc.h; y <= sp.y + sp.h; y += (rc.h + padding.h)) {
            for (const int& padXTip : padXTips) {
              if (y + padding.h > sp.y2())
                render->fillRect(
                  dark, gfx::Rect(padXTip - rc.w, y, rc.w, sp.y2() - y));
              else
                render->fillRect(dark,
                                 gfx::Rect(padXTip - rc.w, y, rc.w, padding.h));
            }
            if (!padXTips.empty()) {
          ...
          if (!hasFlag(Flags::IncludePartialTiles) && (lastX > 0)) {
            if (lastX + padding.w < sp.x2())
              render->fillRect(
                dark,
                gfx::Rect(
                  lastX + padding.w, rc.y, sp.x2() - lastX - padding.w, rc.h));
          }
        }
      }
          ...
          if (!hasFlag(Flags::IncludePartialTiles) && (lastY > 0)) {
            if (lastY + padding.h < sp.y2())
              render->fillRect(
                dark,
                gfx::Rect(
                  rc.x, lastY + padding.h, rc.w, sp.y2() - lastY - padding.h));
          }
        }
      }
    }
      ...
      if (rc.y2() < vp.y2())
        render->fillRect(dark,
                         gfx::Rect(vp.x, rc.y2(), vp.w, vp.y2() - rc.y2()));
      // Left band
      if (rc.x > vp.x)
      ...
      if (rc.x2() < vp.x2())
        render->fillRect(dark,
                         gfx::Rect(rc.x2(), rc.y, vp.x2() - rc.x2(), rc.h));
    }
  }
  ...
  if (hasFlag(Flags::PaddingRulers)) {
    if (hasFlag(Flags::HGrid) && hasFlag(Flags::VGrid)) {
      if (rc.w > 0 && padding.w == 0) {
        for (int x = rc.x + rc.w * 2 + padding.w; x <= sp.x + sp.w;
             x += rc.w + padding.w)
          render->drawLine(gridColor, x, rc.y, x, sp.y2());
        for (int x = rc.x + rc.w + padding.w; x <= sp.x + sp.w;
             x += rc.w + padding.w)
          render->drawLine(gridColor, x, rc.y, x, sp.y2());
      }

      if (rc.h > 0 && padding.h == 0) {
        for (int y = rc.y + rc.h * 2 + padding.h; y <= sp.y + sp.h;
             y += rc.h + padding.h)
          render->drawLine(gridColor, rc.x, y, sp.x2(), y);
        for (int y = rc.y + rc.h + padding.h; y <= sp.y + sp.h;
             y += rc.h + padding.h)
          render->drawLine(gridColor, rc.x, y, sp.x2(), y);
      }
    }
                                  ...
                                  const bool updateMovingRulers)
{
  ...
  for (int i = 0; i < int(m_rulers.size()); ++i) {
    ...
    if (hitTestRuler(editor, ruler, oppRuler, mousePos)) {
      if (!hasFlag(Flags::PaddingRulers) &&
          (((ruler.align() & (LEFT | TOP)) &&
            ruler.position() > oppRuler.position()) ||
           ((ruler.align() & (RIGHT | BOTTOM)) &&
            ruler.position() <= oppRuler.position())))
  ...
  if (align == 0 && !hasFlag(Flags::QuickBox)) {
    if (editor
          ->editorToScreen(getBoxBounds().offset(editor->mainTilePosition()))
          .contains(mousePos)) {
                                 ...
                                 const gfx::Point& mousePos)
{
  ...
  pt =
    editor->editorToScreen(pt + gfx::Point(ruler.position(), ruler.position()));
...
CursorType SelectBoxState::cursorFromAlign(const int align) const
{
  switch (align & (LEFT | TOP | RIGHT | BOTTOM)) {
    ...
    case TOP:
      return kSizeNCursor;
    case TOP | LEFT:
      return kSizeNWCursor;
    case TOP | RIGHT:
      return kSizeNECursor;
    case BOTTOM:
      return kSizeSCursor;
    case BOTTOM | LEFT:
      return kSizeSWCursor;
    case BOTTOM | RIGHT:
      return kSizeSECursor;
