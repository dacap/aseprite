...
void StandbyState::onActiveToolChange(Editor* editor, tools::Tool* tool)
{
  ...
  if (m_transformSelectionHandlesAreVisible != needDecorators ||
      !editor->layer() || !editor->layer()->isReference()) {
    m_transformSelectionHandlesAreVisible = false;
    editor->invalidate();
  }
}
...
bool StandbyState::onMouseDown(Editor* editor, MouseMessage* msg)
{
  ...
  if (m_decorator->getSymmetryHandles(editor, handles)) {
    for (const auto& handle : handles) {
      if (handle.bounds.contains(msg->position())) {
        auto mode =
          (handle.align & (TOP | BOTTOM) ? app::gen::SymmetryMode::HORIZONTAL :
  ...
  if (clickedInk->isCelMovement()) {
    ...
    if (editor->isAutoSelectLayer()) {
      ...
      ColorPicker picker;
      picker.pickColor(
        site, cursor, editor->projection(), ColorPicker::FromComposition);
      ...
      if (layer->isBackground()) {
        ...
        if (collect.empty()) {
          ...
          try {
            ...
            MovingCelState* newState =
              new MovingCelState(editor, msg, handle, collect);
    ...
    switch (hit.type()) {
      ...
      case EditorHit::SliceCenter:
        if (msg->left()) {
          ...
          if (!hit.slice() ||
              !site.selectedSlices().contains(hit.slice()->id())) {
          ...
          if (popupMenu) {
            ...
            if (!editor->hasSelectedSlices())
              params.set(
                "id", base::convert_to<std::string>(hit.slice()->id()).c_str());
...
bool StandbyState::onDoubleClick(Editor* editor, MouseMessage* msg)
{
  ...
  if (ink->isSelection() &&
    ...
    startDrawingState(
      editor, msg, DrawingType::SelectTiles, pointer_from_msg(editor, msg));
    return true;
  }
  // Show slice properties when we double-click it
    ...
    if (hit.slice()) {
      ...
      Params params;
      params.set("id",
                 base::convert_to<std::string>(hit.slice()->id()).c_str());
...
bool StandbyState::onKeyDown(Editor* editor, KeyMessage* msg)
{
  ...
  Keys keys = KeyboardShortcuts::instance()->getDragActionsFromKeyMessage(
    KeyContext::MouseWheel, msg);
...
bool StandbyState::onUpdateStatusBar(Editor* editor)
{
  ...
  // For eye-dropper
    ...
    Site site = editor->getSite();
    Mask* mask =
      (editor->document()->isMaskVisible() ? editor->document()->mask() : NULL);
    ...
    if (mask)
      buf +=
        fmt::format(" :selsize: {} {}", mask->bounds().w, mask->bounds().h);
    ...
    if ((editor->docPref().show.grid()) ||
      ...
      if (!grid.isEmpty()) {
        ...
        // Show the tile index for a regular layer/grid
          ...
          tileIndexColumns = int(std::ceil(
            double(sprite->bounds().w - grid.origin().x) / grid.tileSize().w));
        }
    ...
    if (editor->docPref().show.slices()) {
      ...
      for (auto slice : editor->document()->sprite()->slices()) {
        ...
        if (key && key->bounds().contains(int(std::floor(spritePos.x)),
                                          int(std::floor(spritePos.y)))) {
          if (++count == 3) {
    ...
    if (drawingState) {
      ...
      drawingState->sendMovementToToolLoop(tools::Pointer(
        pointer ? pointer->point() :
                  editor->screenToEditor(editor->mousePosInDisplay()),
        tools::Vec2(0.0f, 0.0f),
        pointerButton,
...
void StandbyState::startFlipTransformation(Editor* editor,
...
void StandbyState::transformSelection(Editor* editor,
                                      MouseMessage* msg,
                                      HandleType handle)
{
  ...
  try {
    ...
    if (site.layer() && site.layer()->isTilemap() &&
      ...
      tmpImage.reset(new_image_from_mask(
        site, Preferences::instance().experimental.newBlend()));
    }
...
void StandbyState::callEyedropper(Editor* editor, const ui::MouseMessage* msg)
{
  ...
  bool fg =
    (static_cast<tools::PickInk*>(clickedInk)->target() == tools::PickInk::Fg);

  eyedropper->executeOnMousePos(
    UIContext::instance(), editor, msg->position(), fg);
}
                                      ...
                                      const gfx::Point& mouseScreenPos) const
{
  // Move selection edges
  if (
    Preferences::instance().selection.moveEdges() && editor->isActive() &&
    editor->document()->isMaskVisible() &&
                                          ...
                                          const gfx::Point& mouseScreenPos)
{
  ...
  if (ink && ink->isSelection() && editor->document()->isMaskVisible() &&
    ...
    const int angleInt =
      std::clamp<int>(std::floor(8.0 * angle / (2.0 * PI) + 0.5), 0, 8) % 8;
    ...
    if (handle == PivotHandle) {
      ...
      const Cursor* rotated_rotate_cursors[8] = {
        theme->cursors.rotateW(), theme->cursors.rotateNw(),
        theme->cursors.rotateN(), theme->cursors.rotateNe(),
        theme->cursors.rotateE(), theme->cursors.rotateSe(),
      ...
      const Cursor* rotated_skew_cursors[8] = {
        theme->cursors.skewW(), theme->cursors.skewNw(),
