        ...
        if (colorSource) {
          ...
          if (nativeWindow->contentRect().contains(screenPos)) {
            ...
            if (nativeWindow->surface()->bounds().contains(mousePos))
              gfxColor =
                nativeWindow->surface()->getPixel(mousePos.x, mousePos.y);
          }
...
void ColorButton::onPaint(PaintEvent& ev)
{
  ...
  // When the button is not pressed, show the real color
    ...
    if (color.getType() == app::Color::IndexType && editor &&
        editor->sprite() && editor->sprite()->pixelFormat() == IMAGE_INDEXED) {
      m_dependOnLayer = true;

      if (int(editor->sprite()->transparentColor()) == color.getIndex() &&
          editor->layer() && !editor->layer()->isBackground()) {
  ...
  draw_color_button(
    g, rc, color, (doc::ColorMode)m_pixelFormat, hasMouse(), false);
  ...
  fit_bounds(display(),
             ...
             [this, pinned, forcePinned](
               ...
               if (!pinned || (forcePinned && m_hiddenPopupBounds.isEmpty())) {
                 ...
                 winBounds.x = std::clamp(
                   bounds.x, workarea.x, workarea.x2() - winBounds.w);
