...
void ColorTintShadeTone::setShaderParams(SkRuntimeShaderBuilder& builder,
                                         bool main)
{
...
{
  m_hueWithSatValue =
    Preferences::instance().experimental.hueWithSatValueForColorSelector();
  ...
  if (m_paintFlags & BottomBarFlag) {
    ...
    for (int x = 0; x < bottom.w && !stop; ++x) {
      paint.color(color_utils::color_for_ui(
        app::Color::fromHsv((360.0 * x / bottom.w), sat, val)));
...
int ColorTintShadeTone::onNeedsSurfaceRepaint(const app::Color& newColor)
{
  int flags =
    ...
    (cs_double_diff(m_color.getHsvHue(), newColor.getHsvHue()) ? MainAreaFlag :
                                                                 0) |
