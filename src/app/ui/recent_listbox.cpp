  ...
  void onPaint(PaintEvent& ev) override
  {
    ...
    if (Preferences::instance().general.showFullPath()) {
      ...
      gfx::Rect detailsBounds(
        bounds.x + textSize.w, bounds.y, bounds.w - textSize.w, bounds.h);
...
{
  m_recentFilesConn =
    App::instance()->recentFiles()->Changed.connect([this] { rebuildList(); });
...
void RecentFilesListBox::onUpdateRecentListFromUIItems(
...
void RecentFoldersListBox::onUpdateRecentListFromUIItems(
  const base::paths& pinnedPaths, const base::paths& recentPaths)
{
