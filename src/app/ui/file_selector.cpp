  ...
  bool onProcessMessage(ui::Message* msg) override
  {
    switch (msg->type()) {
      ...
      case kKeyDownMessage: {
        ...
        bool back =
          (msg->cmdPressed() && (unicode == '[' || scancode == kKeyOpenbrace));
        bool forward =
          (msg->cmdPressed() && (unicode == ']' || scancode == kKeyClosebrace));
#else
...
#endif
        bool refresh =
          (scancode == kKeyF5 || (msg->ctrlPressed() && scancode == kKeyR) ||
  ...
  , m_navigationLocked(false)
{
  ...
  m_fileList->CurrentFolderChanged.connect(
    [this] { onFileListCurrentFolderChanged(); });
}
                        ...
                        base::paths& output)
{
  ...
  FILESEL_TRACE("FILESEL: Start folder '%s' (%p)\n",
                start_folder_path.c_str(),
                start_folder);

  {
    const gfx::Size workareaSize =
      ui::Manager::getDefault()->display()->workareaSizeUIScale();
  ...
  fileType()->addItem(new CustomFileExtensionItem(
    Strings::file_selector_all_formats(), allExtensions));
  ...
  for (const auto& e : allExtensions) {
    ...
    fileType()->addItem(
      new CustomFileExtensionItem(e + " files", base::paths{ e }));
  }
  // All files
  fileType()->addItem(new CustomFileExtensionItem(
    Strings::file_selector_all_files(),
  ...
  if (closer() == ok || closer() == m_fileList) {
    ...
    if (fn == "..") {
      ...
      if (!enter_folder) {
        ...
        if (base::is_path_separator(
              *fn.begin())) {  // absolute path (UNIX style)
#ifdef _WIN32
        ...
        // does the file-name entry have colon?
                 ...
                 std::string::npos) {  // absolute path on Windows
          if (fn.size() == 2 && fn[1] == ':') {
    ...
    if (enter_folder && enter_folder->isFolder() &&
    ...
    if (!enter_folder) {
      const bool has_invalid_char = (fn.find(':') != std::string::npos ||
    ...
    if (m_type == FileSelectorType::Save && !buf.empty() &&
        base::get_file_extension(buf).empty()) {
      buf += '.';
...
void FileSelector::updateLocation()
{
  ...
  if (!recent->pinnedFolders().empty()) {
    auto sep =
      new SeparatorInView(Strings::file_selector_pinned_folders(), HORIZONTAL);
  ...
  if (!recent->recentFolders().empty()) {
    auto sep =
      new SeparatorInView(Strings::file_selector_recent_folders(), HORIZONTAL);
    sep->setMinSize(gfx::Size(0, sep->sizeHint().h * 2));
    location()->addItem(sep);
...
void FileSelector::updateNavigationButtons()
{
  ...
  goForwardButton()->setEnabled(
    navigation_history.size() > 1 && navigation_position.is_valid() &&
...
void FileSelector::onLocationCloseListBox()
{
  ...
  IFileItem* fileItem =
    (comboFileItem ? comboFileItem->getFileItem() : nullptr);
...
void FileSelector::onFileTypeChange()
{
  base::paths exts;
  auto* selExtItem =
    dynamic_cast<CustomFileExtensionItem*>(fileType()->getSelectedItem());
  if (selExtItem)
  ...
  if (exts != m_fileList->extensions()) {
    ...
    if (m_type == FileSelectorType::Open ||
        m_type == FileSelectorType::OpenMultiple) {
      const base::paths& allExtensions =
        dynamic_cast<CustomFileExtensionItem*>(fileType()->getItem(0))
          ->extensions();
      std::string k = merge_paths(allExtensions);
