...
protected:
  void onItemChange(Item* item) override
  {
    ...
    Command* discardBrush =
      Commands::instance()->byId(CommandId::DiscardBrush());
    ...
    buttonset.ItemChange.connect(
      [&buttonset, &toolPref](ButtonSet::Item* item) {
        switch (buttonset.selectedItem()) {
          case 0:
            toolPref.floodfill.pixelConnectivity(
              app::gen::PixelConnectivity::FOUR_CONNECTED);
            break;
          case 1:
            toolPref.floodfill.pixelConnectivity(
...
protected:
  void onItemChange(Item* item) override
  {
    ...
    AppMenus::instance()->getInkPopupMenu()->showPopup(
      gfx::Point(bounds.x, bounds.y2()), display());
...
private:
  ...
  void onShowMenu()
  {
    ...
    Menu menu;
    MenuItem reverse(Strings::context_bar_reverse_shade()),
    ...
    if (!m_shades.empty()) {
      ...
      for (const Shade& shade : m_shades) {
        ...
        shadeWidget->Click.connect(
          [&](ColorShades::ClickEvent&) { m_shade.setShade(shade); });

        auto close = new IconButton(theme->parts.iconClose());
        close->InitTheme.connect([close] {
          close->setBgColor(SkinTheme::get(close)->colors.menuitemNormalFace());
        });
        close->initTheme();
  ...
  void loadShades()
  {
    ...
    for (int i = 0; i < n; ++i) {
      ...
      Shade shade =
        shade_from_string(get_config_string("shades", buf.c_str(), ""));
      if (shade.size() >= 2)
  ...
  void saveShades()
  {
    ...
    for (int i = 0; i < n; ++i) {
      buf = fmt::format("shade{}", i);
      set_config_string(
        "shades", buf.c_str(), shade_to_string(m_shades[i]).c_str());
    }
  }
...
public:
  InkOpacityField()
    : AlphaEntry(AlphaSlider::Type::OPACITY)
  {
  }
...
private:
  void onPopup()
  {
    ...
    Menu menu;
    MenuItem opaque(Strings::context_bar_opaque()),
...
public:
  RotAlgorithmField()
  {
    ...
    // addItem() generates an onChange() event).
    m_lockChange = true;
    addItem(new Item(Strings::context_bar_fast_rotation(),
                     tools::RotationAlgorithm::FAST));
    addItem(new Item(Strings::context_bar_rotsprite(),
                     tools::RotationAlgorithm::ROTSPRITE));
    m_lockChange = false;
...
public:
  ...
  void setupTooltips(TooltipManager* tooltipManager)
  {
    ...
    tooltipManager->addTooltipFor(
      at(0), Strings::context_bar_drop_pixel(), BOTTOM);
    tooltipManager->addTooltipFor(
      at(1), Strings::context_bar_cancel_drag(), BOTTOM);
  }
...
public:
  ...
  {
    tooltipManager->addTooltipFor(
      at(0), Strings::symmetry_toggle_horizontal(), BOTTOM);
    tooltipManager->addTooltipFor(
      at(1), Strings::symmetry_toggle_vertical(), BOTTOM);
    tooltipManager->addTooltipFor(
      at(2), Strings::symmetry_show_options(), BOTTOM);
  }
  ...
  void updateWithCurrentDocument()
  {
    ...
    at(0)->setSelected(int(docPref.symmetry.mode()) &
                           int(app::gen::SymmetryMode::HORIZONTAL) ?
                         true :
                         false);
    at(1)->setSelected(int(docPref.symmetry.mode()) &
...
private:
  void onItemChange(Item* item) override
  {
    ...
    if (app::gen::SymmetryMode(mode) != docPref.symmetry.mode()) {
      ...
      MenuItem resetToViewCenter(
        Strings::symmetry_reset_position_to_view_center());
...
public:
    ...
    , m_action(2)
  {
    ...
    m_sel.ItemChange.connect(
      [this](ButtonSet::Item* item) { onSelAction(m_sel.selectedItem()); });

    m_combobox.setEditable(true);
    m_combobox.setExpansive(true);
    m_combobox.setMinSize(gfx::Size(256 * guiscale(), 0));

    m_action.addItem(theme->parts.iconUserData(),
                     theme->styles.buttonsetItemIconMono());
    m_action.addItem(theme->parts.iconClose(),
                     theme->styles.buttonsetItemIconMono());
...
private:
  ...
  {
    const bool visible =
      (m_doc && m_doc->sprite() && !m_doc->sprite()->slices().empty());
    const bool relayout =
      (visible != m_combobox.isVisible() || visible != m_action.isVisible());
...
ContextBar::ContextBar(TooltipManager* tooltipManager, ColorBar* colorBar)
{
  ...
  addChild(m_ditheringSelector =
             new DitheringSelector(DitheringSelector::SelectMatrix));
  m_ditheringSelector->setUseCustomWidget(
    false);  // Disable custom widget because the context bar is too small
  ...
  m_alphaRangeConn =
    pref.range.opacity.AfterChange.connect([this] { onOpacityRangeChange(); });
...
void ContextBar::updateForTool(tools::Tool* tool)
{
  ...
  if (toolPref) {
    m_sizeConn =
      brushPref->size.AfterChange.connect([this] { onBrushSizeChange(); });
    m_angleConn =
      brushPref->angle.AfterChange.connect([this] { onBrushAngleChange(); });
    ...
    m_contiguousConn = toolPref->contiguous.AfterChange.connect(
      [this] { onToolSetContiguous(); });
  }
  ...
  const bool isEyedropper = tool && (tool->getInk(0)->isEyedropper() ||
                                     tool->getInk(1)->isEyedropper());

  // True if the current tool is move tool.
  const bool isMove = tool && (tool->getInk(0)->isCelMovement() ||
                               tool->getInk(1)->isCelMovement());

  // True if the current tool is slice tool.
  const bool isSlice =
    tool && (tool->getInk(0)->isSlice() || tool->getInk(1)->isSlice());
  ...
  const bool hasSelectOptions =
    tool && (tool->getInk(0)->isSelection() || tool->getInk(1)->isSelection());
  ...
  m_brushType->setVisible(supportOpacity &&
                          (!isFloodfill || (isFloodfill && !withDithering)));
...
// static
doc::BrushRef ContextBar::createBrushFromPreferences(
...
{
  Commands::instance()->add(new QuickCommand(CommandId::ShowBrushes(),
                                             [this] { this->showBrushes(); }));

  Commands::instance()->add(new QuickCommand(CommandId::ShowDynamics(),
