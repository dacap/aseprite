...
{
  int x0 =
    (display()->nativeWindow()->pointFromScreen(screenPos).x - bounds().x);
  int newIndex = -1;
      ...
      if (hasCapture() && m_selected) {
        ...
        // We are dragging a tab...
          ...
          if (!bounds().contains(mousePos) &&
            ...
            if (m_delegate)
              result =
                m_delegate->onFloatingTab(this, m_selected->view, screenPos);
        ...
        if (!m_isDragging) {
          ...
          if (m_delegate) {
            ...
            result = m_delegate->onDropTab(
              this, m_selected->view, mouseMsg->screenPosition(), m_dragCopy);
          }
                   ...
                   bool selected)
{
  ...
  theme->paintWidgetPart(
    g, theme->styles.tab(), gfx::Rect(box.x, box.y + dy, box.w, box.h), info);
...

  {
    ...
    if (box.w > 8 * ui::guiscale()) {
      ...
      if (tabColor != gfx::ColorNone) {
        ...
        g->fillRect(
          tabColor,
          gfx::Rect(box.x + dx + 2, box.y + dy + 3, box.w - dx - 2, box.h - 3));
        g->fillRect(
          tabColor,
          gfx::Rect(box.x + dx + 3, box.y + dy + 2, box.w - dx - 3, 1));
  ...
  if (!closeBox.isEmpty()) {
    ...
    if (tabColor != gfx::ColorNone) {
      g->fillRect(
        tabColor,
        gfx::Rect(closeBox.x, closeBox.y + 3, closeBox.w - 3, closeBox.h - 3));
      g->fillRect(tabColor,
                  gfx::Rect(closeBox.x, closeBox.y + 2, closeBox.w - 4, 1));
    }
...
gfx::Rect Tabs::getTabCloseButtonBounds(Tab* tab, const gfx::Rect& box)
{
  ...
  if (box.w - iconW > 32 * ui::guiscale() || tab == m_selected.get())
    return gfx::Rect(
      box.x2() - iconW, box.y + box.h / 2 - iconH / 2, iconW, iconH);
  else
    return gfx::Rect();
}
...
void Tabs::createFloatingOverlay(Tab* tab)
{
  ...
  os::SurfaceRef surface =
    os::instance()->makeRgbaSurface(tab->width, m_tabsHeight);
  ...
  // Fill the surface with pink color
  {
    ...
    surface->drawRect(gfx::Rect(0, 0, surface->width(), surface->height()),
                      paint);
  }
  {
