...
bool FileList::onProcessMessage(Message* msg)
{
  switch (msg->type()) {
      ...
      if (hasCapture()) {
        ...
        for (auto begin = m_list.begin(), end = m_list.end(), it = begin;
          ...
          if ((info.bounds.contains(mousePos)) ||
            ...
            if (m_multiselect && oldSelected != fi &&
                m_selectedItems.size() == m_list.size()) {
              if (msg->shiftPressed() || msg->ctrlPressed() ||
                  msg->cmdPressed()) {
                m_selectedItems[i] = !m_selectedItems[i];
              }
              else {
                std::fill(
                  m_selectedItems.begin(), m_selectedItems.end(), false);
                m_selectedItems[i] = true;
              }
              invalidate();
            }
    ...
    case kTouchMagnifyMessage: {
      setZoom(zoom() +
              4.0 * static_cast<ui::TouchMessage*>(msg)->magnification());
      break;
    }
  }
...
void FileList::paintItem(ui::Graphics* g, IFileItem* fi, const int i)
{
  ...
  if ((!m_multiselect && fi == m_selected) ||
      (m_multiselect && m_selectedItems.size() == m_list.size() &&
       m_selectedItems[i])) {
  ...
  if (fi->isFolder()) {
    if (isListView()) {
      ...
      thumbnail =
        (m_zoom < 4.0 ? theme->parts.folderIconMedium()->bitmapRef(0) :
  ...
  if (!tbounds.isEmpty()) {
    if (thumbnail) {
      tbounds =
        gfx::Rect(0, 0, thumbnail->width(), thumbnail->height()).fitIn(tbounds);
      ...
      tbounds =
        gfx::Rect(
          0, 0, 20 * guiscale(), 2 + 4 * (8.0 - m_zoom) / 8.0 * guiscale())
          .fitIn(tbounds);
...
void FileList::recalcAllFileItemInfo()
{
  ...
  // Redistribute items in X axis
    ...
    for (int y = 0; i < int(m_info.size());) {
      ...
      int j = 0;
      int x = 0;
      for (; j < itemsPerRow && i < int(m_info.size());
           ++j, ++i, x += itemWidth) {
        auto& info = m_info[i];
        ...
        info.text.y += deltay;
        info.thumbnail.x =
          info.bounds.x + info.bounds.w / 2 - info.thumbnail.w / 2;
...
FileList::ItemInfo FileList::calcFileItemInfo(int i) const
{
  ...
  int rowHeight =
    textHeight + (withThumbnails ? 8 * m_zoom + 2 * guiscale() : 0);

  ItemInfo info;
  info.text = gfx::Rect(0, 0 + rowHeight * i, len + 4 * guiscale(), textHeight);
  if (withThumbnails) {
    info.thumbnail = gfx::Rect(
      0, info.text.y, 8 * m_zoom * guiscale(), 8 * m_zoom * guiscale());
