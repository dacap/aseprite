...
private:
  void onPaint(PaintEvent& ev) override
  {
    ...
    if (m_image) {
      ...
      os::SurfaceRef sur =
        os::instance()->makeRgbaSurface(m_image->width(), m_image->height());
  ...
  void onSizeHint(SizeHintEvent& ev) override
  {
    ...
    if (m_image) {
      ...
      ev.setSizeHint(sz.w + 4 + m_image->width(),
  ...
  void onSelect(bool selected) override
  {
    ...
    try {
      m_image.reset(render_text(m_filename,
                                16,
  ...
  , m_popup(new gen::FontPopup())
{
  ...
  std::sort(
    files.begin(), files.end(), [](const std::string& a, const std::string& b) {
      return base::utf8_icmp(base::get_file_title(a), base::get_file_title(b)) <
             0;
    });
...
void FontPopup::showPopup(Display* display, const gfx::Rect& buttonBounds)
{
  ...
  setHotRegion(gfx::Region(
    gfx::Rect(boundsOnScreen()).enlarge(32 * guiscale() * display->scale())));
