  ...
  , m_timer(100, this)
{
  ...
  m_appConn = App::instance()->ColorSpaceChange.connect(
  ...
  switch (msg->type()) {
    ...
    case kMouseMoveMessage: {
      ...
      if (msg->type() == kMouseDownMessage) {
        ...
        m_capturedInMain =
          (hasCapture() && !m_capturedInMain && !m_capturedInBottom);
      }
...
void ColorSelector::onPaint(ui::PaintEvent& ev)
{
  ...
  if (buildEffects()) {
    ...
    canvas->save();
    {
      ...
      // Main area
      gfx::Rect rc2(
        0, 0, rc.w, std::max(1, rc.h - bottomBarBounds.h - alphaBarBounds.h));
