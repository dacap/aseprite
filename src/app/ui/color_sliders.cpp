      ...
      switch (m_channel) {
        ...
        case ColorSliders::Channel::HsvHue:
          color = color_utils::color_for_ui(app::Color::fromHsv(
            360.0 * x / w, m_color.getHsvSaturation(), m_color.getHsvValue()));
          break;
        case ColorSliders::Channel::HsvSaturation:
          color = color_utils::color_for_ui(app::Color::fromHsv(
            m_color.getHsvHue(), double(x) / double(w), m_color.getHsvValue()));
          break;
                             ...
                             const int relMax)
{
  ...
  item.box = new HBox();
  item.absSlider =
    (channel != Channel::Alpha ? new Slider(absMin, absMax, 0) :
...
void ColorSliders::syncRelHsvHslSliders()
{
  ...
  if (m_items[Channel::HsvHue].show) {
    ...
    setRelSliderValue(Channel::HslSaturation,
                      getRelSliderValue(Channel::HsvSaturation));
    setRelSliderValue(Channel::HslLightness,
                      getRelSliderValue(Channel::HsvValue));
  }
  // From HSL -> HSV
  else if (m_items[Channel::HslHue].show) {
    setRelSliderValue(Channel::HsvHue, getRelSliderValue(Channel::HslHue));
    setRelSliderValue(Channel::HsvSaturation,
                      getRelSliderValue(Channel::HslSaturation));
    setRelSliderValue(Channel::HsvValue,
                      getRelSliderValue(Channel::HslLightness));
  }
}
...
void ColorSliders::onEntryChange(const Channel i)
{
  ...
  Slider* slider =
    (m_mode == Mode::Absolute ? m_items[i].absSlider : m_items[i].relSlider);
...
void ColorSliders::onControlChange(const Channel i)
{
  ...
  ColorSlidersChangeEvent ev(
    i, m_mode, m_color, m_items[i].relSlider->getValue(), this);
  ColorChange(ev);
}
...
void ColorSliders::onSetColor(const app::Color& color)
{
  ...
  setAbsSliderValue(Channel::HsvSaturation,
                    int(color.getHsvSaturation() * 100.0 + 0.5));
  ...
  setAbsSliderValue(Channel::HslSaturation,
                    int(color.getHslSaturation() * 100.0 + 0.5));
  setAbsSliderValue(Channel::HslLightness,
                    int(color.getHslLightness() * 100.0 + 0.5));
  ...
  switch (channel) {
    ...
    case Channel::HsvValue:
      return app::Color::fromHsv(getAbsSliderValue(Channel::HsvHue),
                                 getAbsSliderValue(Channel::HsvSaturation) /
                                   100.0,
