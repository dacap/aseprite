...
public:
  ...
  void removeExistentStyles(XMLElement* xmlStyle)
  {
    ...
    while (xmlStyle) {
      ...
      auto it =
        std::find_if(m_styles.begin(), m_styles.end(), [styleId](auto& style) {
  ...
  void copyMissingStyles(XMLNode* xmlStyles)
  {
    ...
    for (auto& style : m_styles) {
      LOG(VERBOSE,
          "THEME: Copying <style id='%s'> from default theme\n",
          style->Attribute("id"));
                           ...
                           const std::string& xmlFilename)
{
  ...
  if (!nameStr)
    throw base::Exception(
      "No \"name\" or \"font\" attributes specified on <font>");
  ...
  if (type == "spritesheet") {
    ...
    throw base::Exception(
      "Invalid type=\"%s\" in '%s' for <font name=\"%s\" ...>\n",
      type.c_str(),
      xmlFilename.c_str(),
      name.c_str());
  }
...
void SkinTheme::loadFontData()
{
  ...
  XMLElement* xmlFont =
    handle.FirstChildElement("fonts").FirstChildElement("font").ToElement();
  while (xmlFont) {
  ...
  // Load fonts
  {
    ...
    while (xmlFont) {
      ...
      if (idStr && fontData) {
        ...
        bool mnemonics =
          mnemonicsStr ? (std::string(mnemonicsStr) != "off") : true;
  ...
  // Load parts
  {
    ...
    while (xmlPart) {
      ...
      int w = (xmlPart->Attribute("w") ?
                 scale * strtol(xmlPart->Attribute("w"), nullptr, 10) :
                 0);
      int h = (xmlPart->Attribute("h") ?
                 scale * strtol(xmlPart->Attribute("h"), nullptr, 10) :
                 0);
      ...
      if (!unscaledPart)
        unscaledPart = m_unscaledParts_by_id[part_id] =
          SkinPartPtr(new SkinPart);

      if (w > 0 && h > 0) {
        part->setSpriteBounds(gfx::Rect(x, y, w, h));
        part->setBitmap(0,
                        sliceSheet(part->bitmapRef(0), gfx::Rect(x, y, w, h)));
        unscaledPart->setSpriteBounds(part->spriteBounds() / scale);
        unscaledPart->setBitmap(
          0,
          sliceUnscaledSheet(unscaledPart->bitmapRef(0),
                             unscaledPart->spriteBounds()));
      }
      else if (xmlPart->Attribute(
                 "w1")) {  // 3x3-1 part (NW, N, NE, E, SE, S, SW, W)
        ...
        part->setBitmap(
          0, sliceSheet(part->bitmapRef(0), gfx::Rect(x, y, w1, h1)));  // NW
        part->setBitmap(
          1,
          sliceSheet(part->bitmapRef(1), gfx::Rect(x + w1, y, w2, h1)));  // N
        part->setBitmap(2,
                        sliceSheet(part->bitmapRef(2),
                                   gfx::Rect(x + w1 + w2, y, w3, h1)));  // NE
        part->setBitmap(
          3,
          sliceSheet(part->bitmapRef(3),
                     gfx::Rect(x + w1 + w2, y + h1, w3, h2)));  // E
        part->setBitmap(
          4,
          sliceSheet(part->bitmapRef(4),
                     gfx::Rect(x + w1 + w2, y + h1 + h2, w3, h3)));  // SE
        part->setBitmap(
          5,
          sliceSheet(part->bitmapRef(5),
                     gfx::Rect(x + w1, y + h1 + h2, w2, h3)));  // S
        part->setBitmap(6,
                        sliceSheet(part->bitmapRef(6),
                                   gfx::Rect(x, y + h1 + h2, w1, h3)));  // SW
        part->setBitmap(
          7,
          sliceSheet(part->bitmapRef(7), gfx::Rect(x, y + h1, w1, h2)));  // W
        ...
        unscaledPart->setBitmap(
          0,
          sliceUnscaledSheet(unscaledPart->bitmapRef(0),
                             gfx::Rect(x, y, w1, h1) / scale));
        unscaledPart->setBitmap(
          1,
          sliceUnscaledSheet(unscaledPart->bitmapRef(1),
                             gfx::Rect(x + w1, y, w2, h1) / scale));
        unscaledPart->setBitmap(
          2,
        ...
        unscaledPart->setBitmap(
          ...
          sliceUnscaledSheet(unscaledPart->bitmapRef(4),
                             gfx::Rect(x + w1 + w2, y + h1 + h2, w3, h3) /
                               scale));
        unscaledPart->setBitmap(
          5,
        ...
        unscaledPart->setBitmap(
          7,
          sliceUnscaledSheet(unscaledPart->bitmapRef(7),
                             gfx::Rect(x, y + h1, w1, h2) / scale));
      }
      ...
      if (std::strncmp(part_id, "cursor_", 7) == 0) {
        ...
        int focusx =
          scale * std::strtol(xmlPart->Attribute("focusx"), NULL, 10);
        int focusy =
          scale * std::strtol(xmlPart->Attribute("focusy"), NULL, 10);
      ...
      while (xmlLayer) {
        ...
        if (colorId) {
          ...
          if (it != m_colors_by_id.end())
            ...
            throw base::Exception(
              "Color <%s color='%s' ...> was not found in '%s'\n",
              layerName.c_str(),
              colorId,
              xml_filename.c_str());
          }
        ...
        if (partId) {
          ...
          if (it != m_parts_by_id.end()) {
            ...
            throw base::Exception(
              "Part <%s part='%s' ...> was not found in '%s'\n",
              layerName.c_str(),
              partId,
              xml_filename.c_str());
          }
...
static os::SurfaceRef sliceSheet(os::SurfaceRef sheet,
                                 os::SurfaceRef sur,
...
os::SurfaceRef SkinTheme::sliceSheet(os::SurfaceRef sur,
                                     const gfx::Rect& bounds)
{
  return app::skin::sliceSheet(m_sheet, sur, bounds);
}

os::SurfaceRef SkinTheme::sliceUnscaledSheet(os::SurfaceRef sur,
                                             const gfx::Rect& bounds)
{
...
void SkinTheme::paintEntry(PaintEvent& ev)
{
  ...
  drawRect(g,
           bounds,
           (widget->hasFocus() ? (isMiniLook ? parts.sunkenMiniFocused().get() :
...
public:
  DrawEntryTextDelegate(Entry* widget,
                        Graphics* graphics,
                        const gfx::Point& pos,
                        const int h)
    : m_widget(widget)
  ...
  bool preDrawChar(const gfx::Rect& charBounds) override
  {
    ...
    if (charBounds.x2() - m_widget->bounds().x <
        m_widget->clientBounds().x2()) {
      if (m_bg != ColorNone) {
        ...
        gfx::Rect fillThisRect(
          m_lastX - m_widget->bounds().x, m_y, charBounds.x2() - m_lastX, m_h);
  ...
  {
    // Caret
    if (m_state && m_index == m_caret && m_widget->hasFocus() &&
        m_widget->isEnabled()) {
      auto theme = SkinTheme::get(m_widget);
      theme->drawEntryCaret(
        m_graphics, m_widget, m_charStartX - m_widget->bounds().x, m_y);
      m_caretDrawn = true;
    }
...
void SkinTheme::drawEntryText(ui::Graphics* g, ui::Entry* widget)
{
  ...
  DrawEntryTextDelegate delegate(
    widget, g, bounds.origin(), widget->textHeight());
...
void SkinTheme::paintMenuItem(ui::PaintEvent& ev)
{
  ...
  if (widget->isSelected()) {
    os::Surface* icon =
      (widget->isEnabled() ? parts.checkSelected()->bitmap(0) :
  ...
  if (!bar) {
    ...
    // Draw the keyboard shortcut
      ...
      if (appMenuItem->key() && !appMenuItem->key()->accels().empty()) {
        ...
        drawText(
          g, buf.c_str(), fg, ColorNone, widget, pos, widget->align(), 0);
                         ...
                         os::Surface* w)
{
  ...
  g->drawRgbaSurface(nw, rc.x, rc.y);
  {
    IntersectClip clip(
      g,
      Rect(rc.x + nw->width(), rc.y, rc.w - nw->width() - ne->width(), rc.h));
    if (clip) {
  ...
  g->drawRgbaSurface(sw, rc.x, rc.y + rc.h - sw->height());
  {
    IntersectClip clip(
      g,
      Rect(rc.x + sw->width(), rc.y, rc.w - sw->width() - se->width(), rc.h));
    if (clip) {
  ...
  g->drawRgbaSurface(se, rc.x + rc.w - se->width(), rc.y + rc.h - se->height());
  {
    IntersectClip clip(
      g,
      Rect(
        rc.x, rc.y + nw->height(), rc.w, rc.h - nw->height() - sw->height()));
    if (clip) {
      // Left
...
void SkinTheme::drawRect2(
  Graphics* g, const Rect& rc, int x_mid, SkinPart* nw1, SkinPart* nw2)
{
...
void SkinTheme::paintProgressBar(ui::Graphics* g,
                                 const gfx::Rect& rc0,
                                 double progress)
{
  gfx::Color border = colors.text();
  border =
    gfx::rgba(gfx::getr(border), gfx::getg(border), gfx::getb(border), 64);
