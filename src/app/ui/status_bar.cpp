...
class StatusBar::Indicators : public HBox {
  ...
  private:
    ...
    {
      setMinSize(minSize().createUnion(
        Size(m_part->bitmap(0)->width(), m_part->bitmap(0)->height())));
    }
    ...
    void onPaint(ui::PaintEvent& ev) override
    {
      ...
      if (m_colored)
        g->drawColoredRgbaSurface(
          icon, textColor, rc.x, rc.y + rc.h / 2 - icon->height() / 2);
      else
  ...
  private:
    void onPaint(ui::PaintEvent& ev) override
    {
      ...
      g->fillRect(bgColor(), rc);
      draw_tile_button(
        g, Rect(rc.x, rc.y, 32 * guiscale(), rc.h), site, m_tile, false, false);
    }
...
public:
  ...
  void addTileIndicator(doc::tile_t tile)
  {
    if (m_iterator != m_indicators.end()) {
      if ((*m_iterator)->indicatorType() == Indicator::kTile) {
        m_redraw |=
          static_cast<TileIndicator*>(*m_iterator)->updateIndicator(tile);
        ++m_iterator;
        return;
      }
      else
  ...
  void showBackupIcon(BackupIcon icon)
  {
    ...
    if (m_backupIcon != BackupIcon::None) {
      auto theme = SkinTheme::get(this);
      SkinPartPtr part =
        (m_backupIcon == BackupIcon::Normal ? theme->parts.iconSave() :
...
void StatusBar::showDefaultText(Doc* doc)
{
  ...
  if (doc) {
    std::string buf =
      fmt::format("{}  :size: {} {}", doc->name(), doc->width(), doc->height());
...
void StatusBar::showColor(int msecs,
...
{
  AppMenus::instance()->getNewFrameMenu()->showPopup(mousePosInDisplay(),
                                                     display());
}
