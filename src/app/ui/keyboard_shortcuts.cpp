...
std::string get_user_friendly_string_for_wheelaction(
  app::WheelAction wheelAction)
{
...
{
  for (int c = int(app::WheelAction::First); c <= int(app::WheelAction::Last);
       ++c) {
                                      ...
                                      const KeyContext keyContext) const
{
  if (auto keyMsg = dynamic_cast<const KeyMessage*>(msg)) {
    for (const Accelerator& accel : accels()) {
      if (accel.isPressed(
            keyMsg->modifiers(), keyMsg->scancode(), keyMsg->unicodeChar()) &&
...
std::string Key::triggerString() const
{
  switch (m_type) {
    ...
    case KeyType::Action:
      return get_user_friendly_string_for_keyaction(m_action, m_keycontext);
    case KeyType::WheelAction:
    case KeyType::DragAction:
...
void KeyboardShortcuts::importFile(XMLElement* rootElement, KeySource source)
{
  ...
  xmlKey =
    handle.FirstChildElement("quicktools").FirstChildElement("key").ToElement();
  while (xmlKey) {
    const char* tool_id = xmlKey->Attribute("tool");
    const char* tool_key = get_shortcut(xmlKey);
    bool removed = bool_attr(xmlKey, "removed", false);
    ...
    if (tool_id) {
      ...
      if (tool) {
        ...
        if (key && tool_key) {
          LOG(VERBOSE,
              "KEYS: Shortcut for quicktool %s: %s\n",
              tool_id,
              tool_key);
          Accelerator accel(tool_key);
  ...
  // <keyboard><wheel><key>
  xmlKey =
    handle.FirstChildElement("wheel").FirstChildElement("key").ToElement();
  while (xmlKey) {
    const char* action_id = xmlKey->Attribute("action");
    const char* action_key = get_shortcut(xmlKey);
    bool removed = bool_attr(xmlKey, "removed", false);
    ...
    if (action_id) {
      WheelAction action =
        base::convert_to<WheelAction, std::string>(action_id);
      if (action != WheelAction::None) {
        KeyPtr key = this->wheelAction(action);
        if (key && action_key) {
          LOG(VERBOSE,
              "KEYS: Shortcut for wheel action %s: %s\n",
              action_id,
              action_key);
          Accelerator accel(action_key);
  ...
  // <keyboard><drag><key>
  xmlKey =
    handle.FirstChildElement("drag").FirstChildElement("key").ToElement();
  while (xmlKey) {
    const char* action_id = xmlKey->Attribute("action");
    const char* action_key = get_shortcut(xmlKey);
    bool removed = bool_attr(xmlKey, "removed", false);
    ...
    if (action_id) {
      ...
      if (action != WheelAction::None) {
        ...
        if (key && action_key) {
          ...
          LOG(VERBOSE,
              "KEYS: Shortcut for drag action %s: %s\n",
              action_id,
              action_key);
          Accelerator accel(action_key);
  ...
  switch (key->type()) {
    case KeyType::Command: {
      ...
      if (key->keycontext() != KeyContext::Any) {
        elem->SetAttribute(
          "context", base::convert_to<std::string>(key->keycontext()).c_str());
      }
    ...
    case KeyType::WheelAction:
      elem->SetAttribute(
        "action", base::convert_to<std::string>(key->wheelAction()).c_str());
      break;
...
{
  for (KeyPtr& key : m_keys) {
    if (key.get() != newKey && key->keycontext() == keyContext &&
        key->hasAccel(accel) &&
        // Tools can contain the same keyboard shortcut
        (key->type() != KeyType::Tool || newKey == nullptr ||
...
tools::Tool* KeyboardShortcuts::getCurrentQuicktool(tools::Tool* currentTool)
{
  if (currentTool && currentTool->getInk(0)->isSelection()) {
    KeyPtr key =
      action(KeyAction::CopySelection, KeyContext::TranslatingSelection);
...
WheelAction KeyboardShortcuts::getWheelActionFromMouseMessage(
  const KeyContext context, const ui::Message* msg)
{
...
{
  for (int action = int(WheelAction::First); action <= int(WheelAction::Last);
       ++action) {
    // Wheel actions
    auto it = std::find_if(
      m_keys.begin(), m_keys.end(), [action](const KeyPtr& key) -> bool {
        return key->type() == KeyType::WheelAction &&
    ...
    it = std::find_if(
      m_keys.begin(), m_keys.end(), [action](const KeyPtr& key) -> bool {
        return key->type() == KeyType::DragAction &&
               key->wheelAction() == (WheelAction)action;
      });
    if (it == m_keys.end()) {
...
void KeyboardShortcuts::setDefaultMouseWheelKeys(const bool zoomWithWheel)
{
  ...
  key->add(Accelerator(
             zoomWithWheel ? kKeyNoneModifier : kKeyCtrlModifier, kKeyNil, 0),
           KeySource::Original,
           *this);
  m_keys.push_back(key);
  ...
  if (!zoomWithWheel) {
    ...
    key->add(
      Accelerator(kKeyNoneModifier, kKeyNil, 0), KeySource::Original, *this);
    m_keys.push_back(key);
  }

  key = std::make_shared<Key>(WheelAction::HScroll);
  key->add(
    Accelerator(kKeyShiftModifier, kKeyNil, 0), KeySource::Original, *this);
  m_keys.push_back(key);

  key = std::make_shared<Key>(WheelAction::FgColor);
  key->add(
    Accelerator(kKeyAltModifier, kKeyNil, 0), KeySource::Original, *this);
  m_keys.push_back(key);

  key = std::make_shared<Key>(WheelAction::BgColor);
  key->add(Accelerator(
             (KeyModifiers)(kKeyAltModifier | kKeyShiftModifier), kKeyNil, 0),
           KeySource::Original,
           *this);
  m_keys.push_back(key);
  ...
  if (zoomWithWheel) {
    ...
    key->add(
      Accelerator(kKeyCtrlModifier, kKeyNil, 0), KeySource::Original, *this);
    m_keys.push_back(key);

    key = std::make_shared<Key>(WheelAction::Frame);
    key->add(Accelerator((KeyModifiers)(kKeyCtrlModifier | kKeyShiftModifier),
                         kKeyNil,
                         0),
             KeySource::Original,
             *this);
