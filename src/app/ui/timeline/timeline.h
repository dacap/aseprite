...
public:
  ...
  const SelectedLayers& selectedLayers() const
  {
    return m_range.selectedLayers();
  }
  const SelectedFrames& selectedFrames() const
  {
    return m_range.selectedFrames();
  }
...
private:
  ...
  bool validLayer(layer_t layer) const
  {
  ...
  Hit
    m_clk;  // The 'clk' part is where the mouse's button was pressed (maybe for a drag & drop operation)
