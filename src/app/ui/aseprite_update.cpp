  ...
  , m_timer(500, this)
{
  ...
  m_download.DataReceived.connect(
    [this](long total, long now) { onDataReceived(total, now); });
...
void AsepriteUpdate::onBeforeClose(ui::CloseEvent& ev)
{
  if ((m_download.status() != drm::Thread::Status::FINISHED) ||
      (m_installation &&
       m_installation->status() != drm::Thread::Status::FINISHED)) {
...
void AsepriteUpdate::onDataReceived(long total, long now)
{
  ui::execute_from_ui_thread(
    [this, total, now] { progress()->setValue(100 * now / total); });
}
...
void AsepriteUpdate::onDownloadFinished(drm::Package& package)
{
  ui::execute_from_ui_thread([this, package] {
    ...
    m_installation->InstallationFailed.connect(
      [this](drm::LicenseManager::InstallationException& e) {
    ...
    m_installation->InstallationPhaseStarted.connect(
      [this](drm::InstallationPhase phase) {
...
{
  ui::execute_from_ui_thread(
    [this, pctCompleted] { progress()->setValue(pctCompleted); });
}
...
void AsepriteUpdate::onInstallationFailed(
  drm::LicenseManager::InstallationException& e)
{
  ui::execute_from_ui_thread([this, e] { log(e.what()); });
}
