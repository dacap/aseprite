...
public:
  ...
  void deleteBackup()
  {
    ...
    m_task =
      new TaskWidget(TaskWidget::kCannotCancel, [this](base::task_token& t) {
        try {
          // Warning: This is executed from a worker thread
  ...
  void updateText()
  {
    if (!m_task) {
      ...
      setText(
        m_backup->description(Preferences::instance().general.showFullPath()));
    }
  }
  ...
  , m_waitToEnableRefreshTimer(100)
{
  ...
  m_waitToEnableRefreshTimer.Tick.connect(
    [this] { onCheckIfWeCanEnableRefreshButton(); });
...
void DataRecoveryView::fillListWith(const bool crashes)
{
  ...
  for (auto& session : m_dataRecovery->sessions()) {
    ...
    if (first) {
      ...
      auto sep =
        new Separator((crashes ? Strings::recover_files_crash_sessions() :
                                 Strings::recover_files_old_sessions()),
                      HORIZONTAL);
...
void DataRecoveryView::onDelete()
{
  ...
  // Delete one backup
  if (Alert::show(Strings::alerts_delete_selected_backups(int(items.size()))) !=
      1)
    return;  // Cancel
...
bool DataRecoveryView::thereAreCrashSessions() const
{
  for (auto widget : m_listBox.children()) {
    if (auto item = dynamic_cast<const Item*>(widget)) {
      if (item && item->isVisible() && item->session() &&
