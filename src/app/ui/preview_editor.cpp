...
protected:
  ...
  void onSetDecorativeWidgetBounds() override
  {
    ...
    gfx::Size playSize =
      theme->calcSizeHint(this, theme->styles.windowPlayButton());
    gfx::Size closeSize =
    ...
    rect.offset(window->bounds().x2() -
                  ...
                  closeSize.w -
                  theme->styles.windowPlayButton()->margin().width() -
                  playSize.w - style()->margin().right() - centerSize.w,
...
bool PreviewEditorWindow::onProcessMessage(ui::Message* msg)
{
  switch (msg->type()) {
    case kOpenMessage: {
      ...
      defaultBounds.x = mainWindow.x2() - ToolBar::instance()->sizeHint().w -
                        defaultBounds.w - extra;
      defaultBounds.y = mainWindow.y2() - StatusBar::instance()->sizeHint().h -
                        defaultBounds.h - extra;
...
void PreviewEditorWindow::adjustPlayingTag()
{
  ...
  if (miniEditor->isPlaying()) {
    doc::Tag* tag =
      editor->getCustomizationDelegate()->getTagProvider()->getTagByFrame(
        editor->frame(), true);
