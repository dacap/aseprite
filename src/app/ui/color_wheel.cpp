...
app::Color ColorWheel::getMainAreaColor(const int _u,
                                        const int umax,
                                        const int _v,
                                        const int vmax)
{
  ...
  if (m_color.getAlpha() > 0) {
    ...
    for (int i = 0; i < n; ++i) {
      ...
      if (gfx::Rect(umax - (n - i) * boxsize, vmax - boxsize, boxsize, boxsize)
            .contains(pos)) {
    ...
    if (d <= m_wheelRadius) {
      ...
      if (m_discrete) {
        ...
        if (normalizedDistance < 1.0 / 6.0)
          angle = 0;
        normalizedDistance =
          (std::floor((normalizedDistance) * 6.0 + 1.0) - 1) / 5.0;
        int blueAngleDegrees =
          90.0 * (6.0 - std::floor(normalizedDistance * 6.0)) / 5.0;
        blueAngle = PI * blueAngleDegrees / 180.0;
  ...
  if (m_color.getAlpha() > 0) {
    if (m_colorModel == ColorModel::NORMAL_MAP) {
      ...
      if (normalizedBlue > approximationThreshold) {
        ...
        double dist = std::sqrt(normalizedRed * normalizedRed +
                                          ...
                                          bool& stop)
{
  ...
  if (m_paintFlags & BottomBarFlag) {
    ...
    for (int x = 0; x < bottom.w && !stop; ++x) {
      paint.color(color_utils::color_for_ui(
...
int ColorWheel::onNeedsSurfaceRepaint(const app::Color& newColor)
{
  return
    ...
    (cs_double_diff(m_color.getHsvHue(), newColor.getHsvHue()) ||
         cs_double_diff(m_color.getHsvSaturation(),
...
app::Color ColorWheel::getColorInHarmony(int j) const
{
  ...
  return app::Color::fromHsv(
    std::fmod(hue, 360), std::clamp(sat, 0.0, 1.0), m_color.getHsvValue());
}
