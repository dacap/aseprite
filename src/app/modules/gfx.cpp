                       ...
                       const bool drag)
{
  ...
  draw_color(
    g,
    Rect(
      rc.x + 1 * scale, rc.y + 1 * scale, rc.w - 2 * scale, rc.h - 2 * scale),
    color,
    colorMode);
               ...
               doc::tile_t tile)
{
  ...
  convert_image_to_surface(
    tileImage.get(), get_current_palette(), surface.get(), 0, 0, 0, 0, w, h);
  ...
  if (w > rc.w && h > rc.h) {
    sampling =
      os::Sampling(os::Sampling::Filter::Linear, os::Sampling::Mipmap::Nearest);
  }
  ...
  g->setMatrix(m);
  g->drawRgbaSurface(
    surface.get(), -g->getInternalDeltaX(), -g->getInternalDeltaY());
  g->restore();
}
                       ...
                       const app::Color& color)
{
  ...
  for (int x = 0; x < rc.w; ++x) {
    ...
    g->drawVLine(app::color_utils::color_for_ui(
                   app::Color::fromImage(IMAGE_RGB, odd ? c2 : c1)),
                 rc.x + x,
                 rc.y,
                 mid);
    g->drawVLine(app::color_utils::color_for_ui(
                   app::Color::fromImage(IMAGE_RGB, odd ? c1 : c2)),
                 rc.x + x,
                 rc.y + mid,
                 rc.h - mid);
  }
                       ...
                       const app::Color& color)
{
  ...
  for (int x = 0; x < rc.w; ++x) {
    ...
    paint.color(app::color_utils::color_for_ui(
      app::Color::fromImage(IMAGE_RGB, odd ? c2 : c1)));
