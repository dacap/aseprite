...
public:
  ...
  const doc::SelectedLayers& selectedLayers() const
  {
    return m_range.selectedLayers();
  }
  const doc::SelectedFrames& selectedFrames() const
  {
  ...
  const doc::SelectedObjects& selectedSlices() const
  {
    return m_selectedSlices;
  }
