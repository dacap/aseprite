...
public:
  ...
  void transformPoint(ToolLoop* loop, const Stroke::Pt& pt) override
  {
    ...
    if (m_useDynamics) {
      ...
      if (m_hasDynamicGradient && m_dynamics.ditheringMatrix.rows() == 1 &&
        ...
        auto rgbaGradient = [t, ti](color_t a, color_t b) -> color_t {
          if (rgba_geta(a) == 0)
            return doc::rgba(
              rgba_getr(b), rgba_getg(b), rgba_getb(b), int(t * rgba_geta(b)));
          else if (rgba_geta(b) == 0)
            return doc::rgba(
              rgba_getr(a), rgba_getg(a), rgba_getb(a), int(ti * rgba_geta(a)));
          else
        ...
        switch (loop->sprite()->pixelFormat()) {
          ...
          case IMAGE_GRAYSCALE:
            ...
            break;
          case IMAGE_INDEXED: {
            int maskIndex = (loop->getLayer()->isBackground() ?
                               -1 :
            ...
            a = loop->getRgbMap()->mapColor(
              rgba_getr(a), rgba_getg(a), rgba_getb(a), rgba_geta(a));
            break;
          }
        }
      ...
      int size = std::clamp(
        int(pt.size), int(Brush::kMinBrushSize), int(Brush::kMaxBrushSize));
      ...
      if ((brush->size() != size) ||
        ...
        // TODO cache brushes
        BrushRef newBrush =
          std::make_shared<Brush>(m_origBrushType, size, angle);
    ...
    if (int(loop->getTiledMode()) & int(TiledMode::X_AXIS)) {
      int wrappedPatternOriginX =
        wrap_value(brush->patternOrigin().x, loop->sprite()->width()) %
        brush->bounds().w;
      brush->setPatternOrigin(
        gfx::Point(wrappedPatternOriginX, brush->patternOrigin().y));
      x = wrap_value(x, loop->sprite()->width());
    }
    if (int(loop->getTiledMode()) & int(TiledMode::Y_AXIS)) {
      int wrappedPatternOriginY =
        wrap_value(brush->patternOrigin().y, loop->sprite()->height()) %
        brush->bounds().h;
      brush->setPatternOrigin(
        gfx::Point(brush->patternOrigin().x, wrappedPatternOriginY));
    ...
    if (!compressPtr) {
      switch (symmetryMode) {
        case gen::SymmetryMode::NONE: {
          compressPtr.reset(new CompressedImage(
            m_lastBrush->image(), m_lastBrush->maskBitmap(), false));
          break;
        }
        case gen::SymmetryMode::HORIZONTAL:
        case gen::SymmetryMode::VERTICAL: {
          std::unique_ptr<Image> tempImage(
            Image::createCopy(m_lastBrush->image()));
          ...
          doc::algorithm::flip_image(
            tempImage.get(), tempImage->bounds(), flip);
          compressPtr.reset(new CompressedImage(
            tempImage.get(), m_lastBrush->maskBitmap(), false));
          break;
        }
