...
public:
  ...
  virtual void prepareUForPointShapeSlicedScanline(ToolLoop* loop,
                                                   bool leftSlice,
                                                   int x1)
  {
  }
...
public:
  ...
  {
    m_dstAddress =
      (typename ImageTraits::address_t)loop->getDstImage()->getPixelAddress(x1,
                                                                            y);
  }
...
public:
  ...
  {
    m_srcAddress =
      (typename ImageTraits::address_t)loop->getSrcImage()->getPixelAddress(x1,
                                                                            y);
    m_dstAddress =
      (typename ImageTraits::address_t)loop->getDstImage()->getPixelAddress(x1,
                                                                            y);
...
class LockAlphaInkProcessing
  : public DoubleInkProcessing<LockAlphaInkProcessing<ImageTraits>,
                               ImageTraits> {
public:
...
void LockAlphaInkProcessing<RgbTraits>::processPixel(int x, int y)
{
  ...
  *m_dstAddress = doc::rgba(rgba_getr(result),
                            rgba_getg(result),
                            rgba_getb(result),
...
class MergeInkProcessing<IndexedTraits>
  : public DoubleInkProcessing<MergeInkProcessing<IndexedTraits>,
                               IndexedTraits> {
public:
  ...
  {
    m_color =
      (int(loop->getPrimaryColor()) == m_maskIndex ?
...
public:
  ...
  void processPixel(int x, int y)
  {
    ...
    get_neighboring_pixels<RgbTraits>(
      m_srcImage, x, y, 3, 3, 1, 1, m_tiledMode, m_area);
...
public:
  ...
  void processPixel(int x, int y)
  {
    ...
    if (m_area.count > 0) {
      ...
      m_area.a /= 9;
      *m_dstAddress = graya_blender_merge(
...
class BlurInkProcessing<IndexedTraits>
  : public DoubleInkProcessing<BlurInkProcessing<IndexedTraits>,
                               IndexedTraits> {
public:
  BlurInkProcessing(ToolLoop* loop)
    : m_palette(loop->getPalette())
...
public:
  ReplaceInkProcessing(ToolLoop* loop)
  {
    ...
    if (loop->getLayer()->isBackground()) {
      switch (loop->sprite()->pixelFormat()) {
        case IMAGE_RGB:
          m_color2 |= rgba_a_mask;
          break;
        case IMAGE_GRAYSCALE:
          m_color2 |= graya_a_mask;
          break;
      }
    }
    m_opacity = loop->getOpacity();
...
private:
  void pickColorFromArea(int x, int y)
  {
    ...
    pt = wrap_point(
      m_tiledMode, gfx::Size(m_srcImageWidth, m_srcImageHeight), pt, false);
...
void JumbleInkProcessing<IndexedTraits>::processPixel(int x, int y)
{
  ...
  color_t c = rgba_blender_merge(
    *m_srcAddress != 0 ? m_palette->getEntry(*m_srcAddress) : 0, tc, m_opacity);
...
public:
  ...
  {
    int i =
      findIndex(rgba_getr(src), rgba_getg(src), rgba_getb(src), rgba_geta(src));
    if (i < 0)
      return src;
...
public:
    ...
    , m_left(loop->getMouseButton() == ToolLoop::Left)
  {
    ...
    const ColorTarget target((loop->getLayer()->isBackground() ?
    ...
    int i = 0;
    for (app::Color color : shade) {
      m_shadePalette.setEntry(i++,
                              color_utils::color_for_target(color, target));
    }
  }
  ...
  {
    int i = findIndex(
      graya_getv(src), graya_getv(src), graya_getv(src), graya_geta(src));
    if (i < 0)
      return src;
...
public:
  using Base =
    DoubleInkProcessing<ShadingInkProcessing<ImageTraits>, ImageTraits>;
...
class GradientInkProcessing
  : public GradientRenderer,
    public DoubleInkProcessing<GradientInkProcessing<ImageTraits>,
                               ImageTraits> {
public:
  typedef DoubleInkProcessing<GradientInkProcessing<ImageTraits>, ImageTraits>
    base;
...
template<>
void GradientInkProcessing<RgbTraits>::prepareForStrokes(ToolLoop* loop,
                                                         Strokes& strokes)
{
...
template<>
void GradientInkProcessing<GrayscaleTraits>::prepareForStrokes(ToolLoop* loop,
                                                               Strokes& strokes)
{
  color_t c0 = loop->getPrimaryColor();
  color_t c1 = loop->getSecondaryColor();
...
void GradientInkProcessing<GrayscaleTraits>::processPixel(int x, int y)
{
  ...
  *m_dstAddress =
    graya_blender_normal(*m_srcAddress, doc::graya(v, a), m_opacity);
  ++m_tmpAddress;
}

template<>
void GradientInkProcessing<IndexedTraits>::prepareForStrokes(ToolLoop* loop,
                                                             Strokes& strokes)
{
...
class BrushInkProcessingBase
  : public DoubleInkProcessing<BrushInkProcessingBase<ImageTraits>,
                               ImageTraits> {
public:
  BrushInkProcessingBase(ToolLoop* loop)
  {
    ...
    m_brushImage =
      (m_brush->patternImage() ? m_brush->patternImage() : m_brush->image());
                            ...
                            int y) override
  {
    if (m_patternAlign != BrushPattern::ALIGNED_TO_SRC) {
      ...
      // Case: during brush preview of PaintBucket Tool
        ...
        m_u = ((m_brush->patternOrigin().x % loop->sprite()->width()) -
               loop->getCelOrigin().x) %
              m_width;
        m_v = ((m_brush->patternOrigin().y % loop->sprite()->height()) -
               loop->getCelOrigin().y) %
              m_height;
      }
    }
  ...
  void prepareVForPointShape(ToolLoop* loop, int y) override
  {
    if (m_patternAlign == doc::BrushPattern::ALIGNED_TO_SRC) {
      ...
      if (y / spriteH > 0)
        ...
        m_v =
          (m_brush->patternOrigin().y + m_height - (y / spriteH) * spriteH) %
          m_height;
      else
        // 'y' is inside of the center tile.
        m_v =
          ((m_brush->patternOrigin().y % spriteH) - loop->getCelOrigin().y) %
          m_height;
    }
  }
  ...
  void prepareUForPointShapeSlicedScanline(ToolLoop* loop,
                                           bool leftSlice,
                                           int x1) override
  {
    if (m_patternAlign == doc::BrushPattern::ALIGNED_TO_SRC) {
...
template<>
bool BrushInkProcessingBase<RgbTraits>::preProcessPixel(int x,
                                                        int y,
                                                        color_t* result)
{
...
template<>
bool BrushInkProcessingBase<GrayscaleTraits>::preProcessPixel(int x,
                                                              int y,
                                                              color_t* result)
{
  if (!alignPixelPoint(x, y))
...
template<>
bool BrushInkProcessingBase<IndexedTraits>::preProcessPixel(int x,
                                                            int y,
                                                            color_t* result)
{
  if (!alignPixelPoint(x, y))
...
public:
  BrushSimpleInkProcessing(ToolLoop* loop)
...
public:
  BrushLockAlphaInkProcessing(ToolLoop* loop)
    : BrushInkProcessingBase<ImageTraits>(loop)
  {
  }
...
void BrushLockAlphaInkProcessing<RgbTraits>::processPixel(int x, int y)
{
  ...
  if (preProcessPixel(x, y, &c))
    *m_dstAddress = doc::rgba(
      rgba_getr(c), rgba_getg(c), rgba_getb(c), rgba_geta(*m_srcAddress));
}
...
public:
  BrushEraserInkProcessing(ToolLoop* loop)
    : BrushInkProcessingBase<ImageTraits>(loop)
  {
  }
...
void BrushShadingInkProcessing<RgbTraits>::processPixel(int x, int y)
{
  ...
  switch (m_brushImage->pixelFormat()) {
    case IMAGE_RGB: {
      ...
      int iBrush = m_shading.findIndex(
...
public:
  BrushCopyInkProcessing(ToolLoop* loop)
    : BrushInkProcessingBase<ImageTraits>(loop)
  {
  }
...
BaseInkProcessing* get_ink_proc(ToolLoop* loop)
{
  switch (loop->sprite()->pixelFormat()) {
    case IMAGE_RGB:
      return new T<RgbTraits>(loop);
    case IMAGE_GRAYSCALE:
      return new T<GrayscaleTraits>(loop);
    case IMAGE_INDEXED:
      return new T<IndexedTraits>(loop);
  }
  ASSERT(false);
  return nullptr;
}
...
BaseInkProcessing* get_ink_proc2(ToolLoop* loop)
{
  switch (loop->getDstImage()->pixelFormat()) {
    ...
    case IMAGE_INDEXED:
      return new T<IndexedTraits>(loop);
    case IMAGE_TILEMAP:
      return new T<TilemapTraits>(loop);
  }
  ASSERT(false);
  return nullptr;
}
