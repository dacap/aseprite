...
public:
  void pressButton(ToolLoop* loop,
                   Stroke& stroke,
                   const Stroke::Pt& pt) override
  {
    m_last = pt;
  }
...
public:
  ...
  void getStatusBarText(ToolLoop* loop,
                        const Stroke& stroke,
...
public:
  ...
  void movement(ToolLoop* loop, Stroke& stroke, const Stroke::Pt& pt) override
  {
    ...
    else if ((int(loop->getModifiers()) &
              int(ToolLoopModifiers::kFromCenter))) {
      int rx = stroke[1].x - m_first.x;
      int ry = stroke[1].y - m_first.y;
      stroke[0].x = m_first.x - rx +
                    (isoAngle && ABS(rx) > ABS(ry) ? SGN(rx) * (rx & 1) : 0);
      stroke[0].y = m_first.y - ry +
                    (isoAngle && ABS(rx) < ABS(ry) ? SGN(ry) * (ry & 1) : 0);
                        ...
                        std::string& text) override
  {
    ...
    text =
      fmt::format(":start: {} {} :end: {} {} :size: {} {} :distance: {:.1f}",
                  stroke[0].x + offset.x,
                  stroke[0].y + offset.y,
                  stroke[1].x + offset.x,
                  stroke[1].y + offset.y,
...
public:
                        ...
                        std::string& text) override
  {
    ...
    gfx::Point offset = loop->statusBarPositionOffset();
    text = fmt::format(
      ":pos: {} {}", stroke[0].x + offset.x, stroke[0].y + offset.y);
  }
};
