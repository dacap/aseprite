...
static int int_attr(const XMLElement* elem,
...
Widget* WidgetLoader::loadWidget(const char* fileName,
                                 const char* widgetId,
                                 ui::Widget* widget)
{
  std::string buf;
                                                ...
                                                Widget* widget)
{
  ...
  if (it != m_typeCreators.end()) {
    ...
    const bool readonly = bool_attr(elem, "readonly", false);
    widget = new AlphaSlider(0,
                             (elem_name == "alphaslider" ?
    ...
    if (!widget) {
      ...
      if (desktop) {
        ...
        widget =
          new Window(Window::WithTitleBar, m_xmlTranslator(elem, "text"));
      }
      else {
    ...
    if (!widget) {
      ...
      widget =
        new ColorButton(Color::fromMask(),
                                                      ...
                                                      Widget* widget)
{
  ...
  if (elem->Attribute("tooltip") && root) {
    ...
    m_tooltipManager->addTooltipFor(
      widgetWithTooltip, m_xmlTranslator(elem, "tooltip"), dir);
  }
  ...
  if (border) {
    ...
    widget->InitTheme.connect(
      [widget, v] { widget->setBorder(gfx::Border(v * guiscale())); });
  }

  if (childspacing) {
    int v = strtol(childspacing, nullptr, 10);
    widget->InitTheme.connect(
      [widget, v] { widget->setChildSpacing(v * guiscale()); });
  }
  ...
  if (minwidth || minheight || maxwidth || maxheight) {
    ...
    widget->InitTheme.connect([widget, minw, minh, maxw, maxh] {
      ...
      widget->setMinMaxSize(
        ...
        gfx::Size(
          (maxw > 0 ? guiscale() * maxw : std::numeric_limits<int>::max()),
...
void WidgetLoader::fillWidgetWithXmlElementAttributesWithChildren(
  const XMLElement* elem, ui::Widget* root, ui::Widget* widget)
{
