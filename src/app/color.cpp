...
Color Color::fromString(const std::string& str)
{
  ...
  if (str != "mask") {
    if (str.find("rgb{") == 0 || str.find("hsv{") == 0 ||
        str.find("hsl{") == 0 || str.find("gray{") == 0) {
      int c = 0;
      ...
      if (str[0] == 'r')
        ...
        color = Color::fromHsv(
          table[0], table[1] / 100.0, table[2] / 100.0, int(table[3]));
...
std::string Color::toHumanReadableString(
  PixelFormat pixelFormat, HumanReadableString humanReadable) const
{
  std::stringstream result;
      ...
      case Color::RgbType:
        if (pixelFormat == IMAGE_GRAYSCALE) {
          ...
          result << "#" << std::hex << std::setfill('0') << std::setw(2)
                 << m_value.rgb.r << std::setw(2) << m_value.rgb.g
...
int Color::getRed() const
{
  switch (getType()) {
    ...
    case Color::HsvType:
      return Rgb(Hsv(m_value.hsv.h, m_value.hsv.s, m_value.hsv.v)).red();

    case Color::HslType:
      return Rgb(Hsl(m_value.hsl.h, m_value.hsl.s, m_value.hsl.l)).red();
...
int Color::getGreen() const
{
  switch (getType()) {
    ...
    case Color::HsvType:
      return Rgb(Hsv(m_value.hsv.h, m_value.hsv.s, m_value.hsv.v)).green();

    case Color::HslType:
      return Rgb(Hsl(m_value.hsl.h, m_value.hsl.s, m_value.hsl.l)).green();
...
int Color::getBlue() const
{
  switch (getType()) {
    ...
    case Color::HsvType:
      return Rgb(Hsv(m_value.hsv.h, m_value.hsv.s, m_value.hsv.v)).blue();

    case Color::HslType:
      return Rgb(Hsl(m_value.hsl.h, m_value.hsl.s, m_value.hsl.l)).blue();
...
double Color::getHsvSaturation() const
{
  switch (getType()) {
    ...
    case Color::HslType:
      return Hsv(Rgb(getRed(), getGreen(), getBlue())).saturation();
...
double Color::getHsvValue() const
{
  switch (getType()) {
    ...
    case Color::HslType:
      return Hsv(Rgb(getRed(), getGreen(), getBlue())).value();
...
double Color::getHslHue() const
{
  switch (getType()) {
    ...
    case Color::RgbType:
      return Hsl(Rgb(m_value.rgb.r, m_value.rgb.g, m_value.rgb.b)).hue();
...
double Color::getHslSaturation() const
{
  switch (getType()) {
    ...
    case Color::RgbType:
      return Hsl(Rgb(m_value.rgb.r, m_value.rgb.g, m_value.rgb.b)).saturation();

    case Color::HsvType:
      return Hsl(Rgb(getRed(), getGreen(), getBlue())).saturation();
...
double Color::getHslLightness() const
{
  switch (getType()) {
    ...
    case Color::RgbType:
      return Hsl(Rgb(m_value.rgb.r, m_value.rgb.g, m_value.rgb.b)).lightness();

    case Color::HsvType:
      return Hsl(Rgb(getRed(), getGreen(), getBlue())).lightness();
...
int Color::getGray() const
{
  switch (getType()) {
    ...
    case Color::RgbType:
      return int(
        255.0 *
        Hsl(Rgb(m_value.rgb.r, m_value.rgb.g, m_value.rgb.b)).lightness());

    case Color::HsvType:
      return int(255.0 * Hsl(Rgb(getRed(), getGreen(), getBlue())).lightness());
    ...
    case Color::IndexType: {
      ...
      if (i >= 0 && i < get_current_palette()->size()) {
        ...
        return int(
          255.0 *
          Hsl(Rgb(rgba_getr(c), rgba_getg(c), rgba_getb(c))).lightness());
      }
      else
        return 0;
    }
...
int Color::getIndex() const
{
  switch (getType()) {
    ...
    case Color::HslType:
    case Color::GrayType: {
      int i = get_current_palette()->findExactMatch(
        getRed(), getGreen(), getBlue(), getAlpha(), -1);
      if (i >= 0)
        return i;
      else
        return get_current_palette()->findBestfit(
          getRed(), getGreen(), getBlue(), getAlpha(), 0);
    }
...
void Color::setAlpha(int alpha)
{
  ...
  switch (getType()) {
    ...
    case Color::IndexType:
      *this = Color::fromRgb(getRed(), getGreen(), getBlue(), alpha);
      break;
  }
}
