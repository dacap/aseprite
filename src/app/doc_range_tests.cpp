...
#define EXPECT_FRAME_ORDER(a, b, c, d)                                         \
  EXPECT_TRUE(expect_frame(a, 0) && expect_frame(b, 1) &&                      \
              expect_frame(c, 2) && expect_frame(d, 3));

#define EXPECT_FRAME_ORDER6(a, b, c, d, e, f)                                  \
  EXPECT_TRUE(expect_frame(a, 0) && expect_frame(b, 1) &&                      \
              expect_frame(c, 2) && expect_frame(d, 3) &&                      \
              expect_frame(e, 4) && expect_frame(f, 5));
...
protected:
  ...
  bool expect_cel(layer_t expected_layer,
                  frame_t expected_frame,
                  layer_t layer,
                  frame_t frame)
  {
    ...
    EXPECT_EQ(expected_color, color)
      << " - expecting layer " << expected_layer << " in " << layer
  ...
  DocRange cels_range(layer_t fromLayer,
                      frame_t fromFrNum,
                      layer_t toLayer,
                      frame_t toFrNum)
  {
...
TEST_F(DocRangeOps, MoveLayersNoOp)
{
  ...
  EXPECT_EQ(
    layers_range(layer1),
    move_range(
      doc.get(), layers_range(layer1), layers_range(layer1), kDocRangeAfter));
  EXPECT_LAYER_ORDER(layer1, layer2, layer3, layer4);
  ...
  EXPECT_EQ(
    layers_range(layer1),
    move_range(
      doc.get(), layers_range(layer1), layers_range(layer2), kDocRangeBefore));
  EXPECT_LAYER_ORDER(layer1, layer2, layer3, layer4);
  EXPECT_FALSE(doc->undoHistory()->canUndo());
  ...
  EXPECT_EQ(
    layers_range(layer4),
    move_range(
      doc.get(), layers_range(layer4), layers_range(layer4), kDocRangeAfter));
  EXPECT_LAYER_ORDER(layer1, layer2, layer3, layer4);
  EXPECT_FALSE(doc->undoHistory()->canUndo());
  ...
  EXPECT_EQ(
    layers_range(layer4),
    move_range(
      doc.get(), layers_range(layer4), layers_range(layer4), kDocRangeBefore));
  EXPECT_LAYER_ORDER(layer1, layer2, layer3, layer4);
  EXPECT_FALSE(doc->undoHistory()->canUndo());
  ...
  EXPECT_EQ(
    layers_range(layer4),
    move_range(
      doc.get(), layers_range(layer4), layers_range(layer3), kDocRangeAfter));
  EXPECT_LAYER_ORDER(layer1, layer2, layer3, layer4);
  EXPECT_FALSE(doc->undoHistory()->canUndo());
...
TEST_F(DocRangeOps, MoveFramesNoOp)
{
  ...
  EXPECT_EQ(frames_range(0, 1),
            move_range(
              doc.get(), frames_range(0, 1), frames_range(0), kDocRangeBefore));
  EXPECT_FRAME_ORDER(0, 1, 2, 3);
  EXPECT_FALSE(doc->undoHistory()->canUndo());
  ...
  EXPECT_EQ(frames_range(0, 1),
            move_range(
              doc.get(), frames_range(0, 1), frames_range(1), kDocRangeBefore));
  EXPECT_FRAME_ORDER(0, 1, 2, 3);
  EXPECT_FALSE(doc->undoHistory()->canUndo());
  ...
  EXPECT_EQ(frames_range(0, 1),
            move_range(
              doc.get(), frames_range(0, 1), frames_range(2), kDocRangeBefore));
  EXPECT_FRAME_ORDER(0, 1, 2, 3);
  EXPECT_FALSE(doc->undoHistory()->canUndo());
  ...
  EXPECT_EQ(frames_range(2, 3),
            move_range(
              doc.get(), frames_range(2, 3), frames_range(2), kDocRangeBefore));
  EXPECT_FRAME_ORDER(0, 1, 2, 3);
  EXPECT_FALSE(doc->undoHistory()->canUndo());
  ...
  EXPECT_EQ(frames_range(2, 3),
            move_range(
              doc.get(), frames_range(2, 3), frames_range(3), kDocRangeBefore));
  EXPECT_FRAME_ORDER(0, 1, 2, 3);
  EXPECT_FALSE(doc->undoHistory()->canUndo());
  ...
  for (int i = 0; i < 2; ++i) {
    for (int frame = 0; frame < 4; ++frame) {
      EXPECT_EQ(
        frames_range(0, 3),
        move_range(
          doc.get(), frames_range(0, 3), frames_range(frame), places[i]));
      EXPECT_FRAME_ORDER(0, 1, 2, 3);
      EXPECT_FALSE(doc->undoHistory()->canUndo());
    }
...
TEST_F(DocRangeOps, MoveLayers)
{
  ...
  EXPECT_EQ(
    layers_range(layer1),
    move_range(
      doc.get(), layers_range(layer1), layers_range(layer2), kDocRangeAfter));
  ...
  EXPECT_EQ(
    layers_range(layer2),
    move_range(
      doc.get(), layers_range(layer2), layers_range(layer1), kDocRangeBefore));
  EXPECT_LAYER_ORDER(layer2, layer1, layer3, layer4);
  doc->undoHistory()->undo();
  EXPECT_LAYER_ORDER(layer1, layer2, layer3, layer4);
  ...
  EXPECT_EQ(
    layers_range(layer2),
    move_range(
      doc.get(), layers_range(layer2), layers_range(layer4), kDocRangeAfter));
  ...
  EXPECT_EQ(
    layers_range(layer2),
    move_range(
      doc.get(), layers_range(layer2), layers_range(layer4), kDocRangeBefore));
...
TEST_F(DocRangeOps, MoveFrames)
{
  ...
  EXPECT_EQ(frames_range(0, 2),
            move_range(
              doc.get(), frames_range(1, 3), frames_range(0), kDocRangeBefore));
...
TEST_F(DocRangeOps, CopyFrames)
{
  ...
  EXPECT_EQ(frames_range(2),
            copy_range(
              doc.get(), frames_range(0), frames_range(2, 3), kDocRangeBefore));
  ...
  EXPECT_EQ(frames_range(0),
            copy_range(
              doc.get(), frames_range(3), frames_range(0, 1), kDocRangeBefore));
  ...
  EXPECT_EQ(frames_range(3, 5),
            copy_range(
              doc.get(), frames_range(0, 2), frames_range(3), kDocRangeBefore));
  ...
  EXPECT_EQ(
    frames_range(0, 2),
    copy_range(
      doc.get(), frames_range(0, 2), frames_range(0, 2), kDocRangeBefore));
  EXPECT_FRAME_COPY3(0, 1, 2, 0, 1, 2, 3);
  doc->undoHistory()->undo();
  EXPECT_FRAME_ORDER(0, 1, 2, 3);
  ...
  EXPECT_EQ(
    frames_range(3, 5),
    copy_range(
      doc.get(), frames_range(0, 2), frames_range(0, 2), kDocRangeAfter));
  EXPECT_FRAME_COPY3(0, 1, 2, 0, 1, 2, 3);
  doc->undoHistory()->undo();
  EXPECT_FRAME_ORDER(0, 1, 2, 3);
}
...
TEST_F(DocRangeOps, MoveRangeWithTags)
{
  ...
  EXPECT_EQ(
    frames_range(2, 3),
    move_range(
      doc.get(), from, frames_range(2), kDocRangeAfter, kFitOutsideTags));
  ...
  EXPECT_EQ(
    frames_range(2, 3),
    move_range(
      doc.get(), from, frames_range(3), kDocRangeBefore, kFitInsideTags));
  EXPECT_FRAME_ORDER6(0, 2, 1, 4, 3, 5);
  EXPECT_EQ(0, a->fromFrame());
  EXPECT_EQ(1, a->toFrame());
