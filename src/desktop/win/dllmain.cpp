...
#define THUMBNAILHANDLER_SHELL_EXTENSION_CLSID                                 \
...
STDAPI DllGetClassObject(REFCLSID clsid, REFIID riid, void** ppv)
{
  ...
  if (IsEqualCLSID(clsid, Global::clsidThumbnailHandler)) {
    ClassFactoryDelegate* delegate =
      new (std::nothrow) ClassFactoryDelegateImpl<ThumbnailHandler>;
    if (!delegate)
...
STDAPI DllRegisterServer()
{
  ...
  if (!GetModuleFileNameW(
        Global::hInstance, dllPath, sizeof(dllPath) / sizeof(dllPath[0])))
    return HRESULT_FROM_WIN32(GetLastError());

  auto hkcu = base::hkey::current_user();
  auto k =
    hkcu.create("Software\\Classes\\CLSID\\" THUMBNAILHANDLER_CLSID_STRING);
...
STDAPI DllUnregisterServer()
{
  ...
  try {
    hkcu.delete_tree(
      "Software\\Classes\\CLSID\\" THUMBNAILHANDLER_CLSID_STRING);
  }
  catch (const base::Win32Exception& ex) {
    hr = HRESULT_FROM_WIN32(ex.errorCode());
  }
