...
bool AsepriteDecoder::decode()
{
  ...
  if (header.width < 1 || header.height < 1) {
    delegate()->error(
      fmt::format("Invalid sprite size {0}x{1}", header.width, header.height));
    return false;
  }

  // Create the new sprite
  std::unique_ptr<doc::Sprite> sprite(std::make_unique<doc::Sprite>(
    doc::ImageSpec(header.depth == 32 ? doc::ColorMode::RGB :
  ...
  sprite->setPixelRatio(
    doc::PixelRatio(header.pixel_width, header.pixel_height));

  // Set grid bounds
  sprite->setGridBounds(gfx::Rect(
    header.grid_x, header.grid_y, header.grid_width, header.grid_height));
      ...
      for (uint32_t c = 0; c < frame_header.chunks; c++) {
        ...
        switch (chunk_type) {
          ...
          case ASE_FILE_CHUNK_LAYER: {
            doc::Layer* newLayer = readLayerChunk(
              &header, sprite.get(), &last_layer, &current_level);
            if (newLayer) {
            ...
            if (last_object_with_user_data) {
              ...
              switch (last_object_with_user_data->type()) {
                ...
                case doc::ObjectType::Tileset:
                  ...
                  // the tags?
                  doc::Tileset* tileset =
                    static_cast<doc::Tileset*>(last_object_with_user_data);
          ...
          default:
            delegate()->incompatibilityError(fmt::format(
              "Warning: Unsupported chunk type {0} (skipping)", chunk_type));
            break;
        }
                                            ...
                                            int* current_level)
{
  ...
  if (layer) {
    if (layer->isImage() &&
      ...
      static_cast<doc::LayerImage*>(layer)->setBlendMode(
    ...
    layer->setFlags(static_cast<doc::LayerFlags>(
      flags & static_cast<int>(doc::LayerFlags::PersistentFlagsMask)));
                    ...
                    const AsepriteHeader* header)
{
  switch (image->pixelFormat()) {
    case doc::IMAGE_RGB:
      read_raw_image_templ<doc::RgbTraits>(f, delegate, image, header);
      break;
                                 ...
                                 const size_t chunk_end)
{
  ...
  while (true) {
    ...
    if (bytes_read == 0) {
      delegate->error(
        fmt::format("Error reading {} bytes of compressed data", input_bytes));
      break;
    }
    ...
    do {
      ...
      if (uncompressed_bytes > 0) {
        ...
        while (y < image->height()) {
          int n =
            std::min(uncompressed_bytes, scanline.size() - scanline_offset);
          if (n > 0) {
                                        ...
                                        const size_t chunk_end)
{
  ...
  if (!layer) {
    delegate()->error(fmt::format("Frame {0} didn't found layer with index {1}",
                                  (int)frame,
                                  (int)layer_index));
    return nullptr;
  }
  if (!layer->isImage()) {
    delegate()->error(fmt::format(
      "Invalid .ase file (frame {0} in layer {1} which does not contain images",
      (int)frame,
      (int)layer_index));
    return nullptr;
  }
  ...
  switch (cel_type) {
    ...
    case ASE_FILE_COMPRESSED_TILEMAP: {
      ...
      if (bitsPerTile != 32) {
        delegate()->incompatibilityError(fmt::format(
          "Unsupported tile format: {0} bits per tile", bitsPerTile));
        break;
      }
      ...
      if (w > 0 && h > 0) {
        ...
        doc::tileset_index tsi =
          static_cast<doc::LayerTilemap*>(layer)->tilesetIndex();
        ...
        doc::transform_image<doc::TilemapTraits>(
          image.get(),
          [ts, tileIDMask, tileIDShift, xflipMask, yflipMask, dflipMask](
            doc::tile_t tile) {
            // Get the tile index.
  ...
  const AsepriteExternalFiles& extFiles)
{
  ...
  if (ntiles < 0 || w < 1 || h < 1) {
    delegate()->error(fmt::format(
      "Error: Invalid tileset (number of tiles={0}, tile size={1}x{2})",
      ntiles,
      w,
      h));
    return nullptr;
  ...
  if (flags & ASE_TILESET_FLAG_EXTERNAL_FILE) {
    const uint32_t extFileId =
      read32();  // filename ID in the external files chunk
    const doc::tileset_index extTilesetId =
  ...
  if (flags & ASE_TILESET_FLAG_EMBEDDED) {
    if (ntiles > 0) {
      ...
      doc::ImageRef alltiles(
        doc::Image::create(sprite->pixelFormat(), w, h * ntiles));
      ...
      for (doc::tile_index i = 0; i < ntiles; ++i) {
        doc::ImageRef tile(doc::crop_image(
          alltiles.get(), 0, i * h, w, h, alltiles->maskColor()));
        tileset->set(i, tile);
      }
...
void AsepriteDecoder::readPropertiesMaps(
...
void AsepriteDecoder::readTilesData(doc::Tileset* tileset,
                                    const AsepriteExternalFiles& extFiles)
{
  ...
  for (doc::tile_index i = 0; i < tileset->size(); i++) {
    ...
    if (chunk_type != ASE_FILE_CHUNK_USER_DATA) {
      ...
      delegate()->error(fmt::format(
        "Warning: Unexpected chunk type {0} when reading tileset index {1}",
        chunk_type,
        i));
      f()->seek(chunk_pos);
      return;
